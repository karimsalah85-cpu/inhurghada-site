...
const cardCopy = {
  en: { fallback: "Red Sea experience", pickup: "Pickup", inquiry: "Price and pickup on request", clear: "Clear price · pickup confirmed", quotation: "Quotation", request: "Request price", from: "From", perPerson: "per person", entrance: "entrance from", person: "person", inquire: "View & inquire", book: "View & book" },
  ar: { fallback: "تجربة على البحر الأحمر", pickup: "الاستلام", inquiry: "السعر والاستلام عند الطلب", clear: "سعر واضح · تأكيد الاستلام", quotation: "عرض سعر", request: "اطلب السعر", from: "ابتداءً من", perPerson: "للشخص", entrance: "رسوم الدخول من", person: "للشخص", inquire: "التفاصيل والاستفسار", book: "التفاصيل والحجز" },
  de: { fallback: "Erlebnis am Roten Meer", pickup: "Abholung", inquiry: "Preis und Abholung auf Anfrage", clear: "Klarer Preis · Abholung bestätigt", quotation: "Angebot", request: "Preis anfragen", from: "Ab", perPerson: "pro Person", entrance: "Eintritt ab", person: "Person", inquire: "Ansehen & anfragen", book: "Ansehen & buchen" },
  ru: { fallback: "Экскурсия на Красном море", pickup: "Трансфер", inquiry: "Цена и трансфер по запросу", clear: "Понятная цена · трансфер подтверждается", quotation: "Расчёт", request: "Запросить цену", from: "От", perPerson: "за человека", entrance: "вход от", person: "чел.", inquire: "Подробнее и запрос", book: "Подробнее и бронирование" },
  pl: { fallback: "Atrakcja nad Morzem Czerwonym", pickup: "Odbiór", inquiry: "Cena i odbiór na zapytanie", clear: "Jasna cena · potwierdzony odbiór", quotation: "Wycena", request: "Zapytaj o cenę", from: "Od", perPerson: "za osobę", entrance: "wstęp od", person: "osoba", inquire: "Zobacz i zapytaj", book: "Zobacz i zarezerwuj" },
  zh: { fallback: "红海体验", pickup: "接送", inquiry: "价格和接送需咨询", clear: "价格透明 · 接送已确认", quotation: "报价", request: "咨询价格", from: "起价", perPerson: "每人", entrance: "门票起价", person: "每人", inquire: "查看并咨询", book: "查看并预订" },
} as const;
    ...
    <article className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg transition duration-300 md:hover:-translate-y-2 md:hover:shadow-2xl">
      ...
      <Link href={link} className="group block">
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="mt-2 flex items-center gap-2 text-sm text-slate-500 sm:mt-3 sm:text-base"><MapPin size={17} /><span>{location}</span></div>
          ...
          <div className="mt-3 flex items-center gap-2 text-sm text-slate-500 sm:mt-5 sm:text-base"><Clock size={17} /><span>{t("everyDay")} · {duration}</span></div>
          {availableTime ? <p className="mt-2 text-xs text-slate-500 sm:mt-3 sm:text-sm">{copy.pickup}: {availableTime}</p> : null}
          <p className="mt-2 flex items-center gap-2 text-xs font-medium text-emerald-700 sm:mt-3 sm:text-sm"><ShieldCheck size={16} />{bookingMode === "inquiry" ? copy.inquiry : copy.clear}</p>
