...
export default function TopServicesPanel({ range }: { range: 7 | 30 | 90 }) {
  ...
  return <section className="mt-8 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div className="flex gap-3"><Trophy className="mt-1 text-amber-600"/><div><h2 className="text-2xl font-bold">Top services</h2><p className="mt-1 text-sm text-slate-500">Confirmed bookings scheduled over the next {range} days, ranked by volume. Trend compares that count against confirmed bookings scheduled in the {range} days before today — a demand pipeline check, not a past-period comparison.</p></div></div><div className="flex rounded-xl border bg-slate-50 p-1">{([7,30,90] as const).map(days=><a key={days} href={`/admin?range=${days}#analytics`} className={`rounded-lg px-3 py-2 text-xs font-black ${range===days?"bg-slate-900 text-white":"text-slate-600"}`}>{days} days</a>)}</div></div>
