...
type Staff = { id: string; email: string; display_name: string | null; role: string; active: boolean; invitation_pending: boolean };
...
export default function TeamManagement({ canManage }: { canManage: boolean }) {
  ...
  async function resendInvitation(item: Staff) {
    setResendingId(item.id);
    setMessage("");
    const response = await fetch(`/api/admin/staff-accounts/${item.id}/resend-invitation`, { method: "POST" });
    const data = await response.json();
    setMessage(response.ok ? `Invitation resent to ${item.email}.` : (data.error || "Could not resend invitation."));
    setResendingId(null);
  ...
  return <>
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.7fr]">
      <section className="rounded-3xl bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Current users</h2><div className="mt-4 divide-y">{staff.map(item => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-4"><div><p className="font-bold">{item.display_name || item.email}</p><p className="text-sm text-slate-500">{item.email}</p>{item.invitation_pending ? <p className="mt-1 text-xs font-bold uppercase tracking-wide text-amber-700">Invitation pending</p> : null}</div><div className="flex flex-wrap gap-2">{canManage && item.active && item.role !== "owner" && item.invitation_pending ? <button disabled={resendingId === item.id} onClick={() => void resendInvitation(item)} className="rounded-lg bg-cyan-50 px-3 py-2 text-sm font-bold text-cyan-800 disabled:opacity-60">{resendingId === item.id ? "Sending…" : "Resend invitation"}</button> : null}<select disabled={!canManage || item.role === "owner"} value={item.role} onChange={event => void update(item, { role: event.target.value })} className="rounded-lg border p-2 text-sm disabled:opacity-60"><option value="owner">owner</option>{roles.map(role => <option key={role}>{role}</option>)}</select><button disabled={!canManage || item.role === "owner"} onClick={() => void update(item, { active: !item.active })} className={`rounded-lg px-3 py-2 text-sm font-bold disabled:opacity-60 ${item.active ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>{item.active ? "Deactivate" : "Reactivate"}</button></div></div>)}</div>{!staff.length ? <p className="mt-4 text-sm text-slate-500">No staff accounts are recorded yet.</p> : null}</section>
