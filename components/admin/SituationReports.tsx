...
export type ReportBooking = {
  ...
  phone?: string | null; customer_email?: string | null;
};

const pageSize = 10;
const pad = (value: number) => String(value).padStart(2, "0");
const cairoDate = (date = new Date()) => date.toLocaleDateString("en-CA", { timeZone: "Africa/Cairo" });
// Cairo's current calendar date, represented as a UTC-midnight Date so day/month arithmetic never drifts into the browser's local timezone.
const cairoCalendarDate = (date = new Date()) => {
  const [year, month, day] = cairoDate(date).split("-").map(Number);
...
export default function SituationReports({ bookings }: { bookings: ReportBooking[] }) {
  ...
  const [dailyMetric, setDailyMetric] = useState<"people" | "bookings">("people");
  ...
  const statistics = useMemo(() => {
    ...
    return {
      active, customers: countDistinctCustomers(active), people: active.reduce((sum, item) => sum + Number(item.guests || 0), 0),
  ...
  const dailySeries = useMemo(() => dailyRows.map(([date, values]) => ({ date, value: values[dailyMetric] })), [dailyRows, dailyMetric]);
  ...
  function setPeriod(next: typeof mode) {
    setMode(next); const date = cairoCalendarDate(); const today = dateValue(date);
    if (next === "today") { setFrom(today); setTo(today); }
    if (next === "next7") { const end = new Date(date); end.setUTCDate(end.getUTCDate() + 6); setFrom(today); setTo(dateValue(end)); }
    if (next === "month") { setFrom(dateValue(new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), 1)))); setTo(today); }
    if (next === "year") { setFrom(dateValue(new Date(Date.UTC(date.getUTCFullYear(), 0, 1)))); setTo(today); }
    setPage(1);
  }
  ...
  return <section className="mt-10 rounded-3xl bg-white p-5 shadow-sm sm:p-7">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[.2em] text-cyan-700">Situation reports & statistics</p><h2 className="mt-2 text-2xl font-black">Know what is happening—and what is next</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Workload, guests, service performance and cash position based on the scheduled service date. Cancelled and refunded bookings are excluded from operating revenue. Archived bookings are excluded entirely — unarchive them first if a period should include them.</p></div><div className="flex flex-wrap gap-2"><a aria-disabled={invalidRange} href={invalidRange ? undefined : `/api/admin/reports?format=pdf&${query}`} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold aria-disabled:pointer-events-none aria-disabled:opacity-40"><Download size={16}/> PDF</a><a aria-disabled={invalidRange} href={invalidRange ? undefined : `/api/admin/reports?format=xlsx&${query}`} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-sm font-bold text-white aria-disabled:pointer-events-none aria-disabled:opacity-40"><Download size={16}/> Excel</a></div></div>
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7"><StatCard icon={<CalendarRange size={18}/>} label="Active bookings" value={String(statistics.active.length)} note={`${statistics.cancelled} cancelled`} /><StatCard icon={<Users size={18}/>} label="Customers" value={String(statistics.customers)} note="Unique bookers" /><StatCard icon={<Users size={18}/>} label="Guests" value={String(statistics.people)} note="All passengers" /><StatCard icon={<TrendingUp size={18}/>} label="Expected" value={formatTotals(statistics.expected)} note="Excludes cancelled/refunded" /><StatCard label="Collected" value={formatTotals(statistics.collected)} note="Marked paid" tone="emerald" /><StatCard label="Outstanding" value={formatTotals(statistics.outstanding)} note="Marked unpaid" tone="amber" /><StatCard label="Cancellation" value={`${statistics.cancellationRate.toFixed(1)}%`} note={`${statistics.cancelled} of ${filtered.length}`} tone={statistics.cancellationRate > 15 ? "rose" : "slate"} /></div>

    <div className="mt-7 grid gap-5 xl:grid-cols-[1.15fr_0.85fr]"><div className="rounded-2xl border border-slate-200 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">Daily workload</h3><p className="mt-1 text-xs text-slate-500">{dailyMetric === "people" ? "Guests" : "Bookings"} scheduled per day on active bookings</p></div><div className="flex items-center gap-3"><div className="flex rounded-lg border border-slate-200 bg-slate-50 p-0.5 text-xs font-bold"><button type="button" onClick={() => setDailyMetric("people")} className={`rounded-md px-2.5 py-1 ${dailyMetric === "people" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>Guests</button><button type="button" onClick={() => setDailyMetric("bookings")} className={`rounded-md px-2.5 py-1 ${dailyMetric === "bookings" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>Bookings</button></div><span className="text-xs font-bold text-slate-400">{dailyRows.length} service days</span></div></div><div className="mt-5"><TrendChart data={dailySeries} label={dailyMetric === "people" ? "Guests" : "Bookings"} formatValue={(value) => String(Math.round(value))} formatDate={(date) => labelDate(date).replace(/, \d{4}/, "")}/></div></div>
    ...
    <div className="mt-8 hidden overflow-x-auto rounded-2xl border border-slate-200 lg:block"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3">Reference</th><th className="p-3">Service</th><th className="p-3">Date</th><th className="p-3">Guests</th><th className="p-3">Booking</th><th className="p-3">Payment</th><th className="p-3 text-right">Amount</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="p-3 font-mono font-bold text-blue-700">{item.reference}</td><td className="max-w-64 truncate p-3" title={item.tour_name || "Private transfer"}>{item.tour_name || "Private transfer"}</td><td className="p-3 whitespace-nowrap">{item.date ? labelDate(item.date) : "To confirm"}</td><td className="p-3">{item.guests || 0}</td><td className="p-3"><Badge value={item.status}/></td><td className="p-3"><Badge value={item.payment_status}/></td><td className="p-3 text-right font-bold whitespace-nowrap">{money(Number(item.amount || 0), item.currency || "USD")}</td></tr>)}</tbody></table>{!rows.length ? <Empty text="No bookings match these report filters."/> : null}</div>
    <div className="mt-8 space-y-3 lg:hidden">{rows.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-sm font-bold text-blue-700">{item.reference}</p><p className="mt-1 font-medium">{item.tour_name || "Private transfer"}</p></div><p className="text-right font-black">{money(Number(item.amount || 0), item.currency || "USD")}</p></div><div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span>{item.date ? labelDate(item.date) : "To confirm"}</span><span>·</span><span>{item.guests || 0} guests</span></div><div className="mt-2 flex gap-2"><Badge value={item.status}/><Badge value={item.payment_status}/></div></article>)}{!rows.length ? <Empty text="No bookings match these report filters."/> : null}</div>
