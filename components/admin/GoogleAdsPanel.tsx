...
import { matchesServiceCampaign, type TopService } from "@/lib/admin-analytics";
...
type ReportResponse =
  ...
  | { configured: true; pendingApproval: true; message: string };

export default function GoogleAdsPanel({ range = 30 }: { range?: 7 | 30 | 90 }) {
  ...
  useEffect(() => { void fetch(`/api/admin/top-services?range=${range}`, { cache: "no-store" }).then(response => response.ok ? response.json() : null).then(data => setTopServices(data?.services || [])); }, [range]);

  const pendingApproval = !!report && "pendingApproval" in report;
  const configured = !!report && "campaigns" in report;
  ...
  return <section id="google-ads" className="mt-8 scroll-mt-6 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
    ...
    {!loading && pendingApproval ? <p role="status" className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">{"message" in report ? report.message : "Google Ads: pending Google approval."}</p> : null}
    ...
    {configured && opportunities.length ? <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="font-black text-amber-950">Google Ads opportunity planner</h3><p className="mt-1 text-sm text-amber-900">High-booking services with no matching campaign, no spend, or no conversions.</p><div className="mt-4 grid gap-3 md:grid-cols-2">{opportunities.map(item=><div key={item.service} className="rounded-xl bg-white p-4 text-sm"><p className="font-black">{item.service}</p><p className="mt-1 text-slate-600">{item.bookings} active bookings · {item.campaigns.length?`${item.campaigns.length} underperforming matched campaign(s)`:"No matching campaign"}</p></div>)}</div></div> : null}
