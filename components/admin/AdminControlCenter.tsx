...
const empty: Record<Resource, Record<string, unknown>> = {
  content: { content_type: "blog", slug: "", locale: "en", status: "draft", listing_status: "active", title: "", excerpt: "", body: "", price: "", originalPrice: "", packagePrice: "", adultPrice: "", childPrice: "", infantPrice: "", duration: "", location: "", availableTimes: "", priceUnit: "", seo_title: "", seo_description: "", featured_image: "", publish_at: "" },
  media: { storage_path: "", public_url: "", file_name: "", mime_type: "image/jpeg", alt_text: "", credit: "", source_url: "", creator: "", license_type: "", license_url: "", attribution_text: "", attribution_required: false, rights_status: "unverified", authenticity: "unknown", focal_x: "0.5", focal_y: "0.5" },
...
export default function AdminControlCenter({ initialTab = "content", variant = "content" }: { initialTab?: Resource; variant?: "content" | "trips" }) {
  ...
  async function updateTripStatus(item: RecordValue, listingStatus: string) {
    const id = String(item.id || "");
    if (!id) return;
    setBusy(true); setError(""); setNotice("");
    try {
      const response = await fetch(`/api/admin/control-center/content/${encodeURIComponent(id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ listing_status: listingStatus }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Could not update trip status.");
      setNotice(`${item.title} is now ${listingStatus}. Public pages and the sitemap were refreshed.`);
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not update trip status."); }
    finally { setBusy(false); }
  }
...
function TripStatusPanel({ items, busy, update, edit }: { items: RecordValue[]; busy: boolean; update: (item: RecordValue, status: string) => Promise<void>; edit: (item: RecordValue) => void }) {
  const [query, setQuery] = useState("");
  const normalized = query.trim().toLowerCase();
  const visible = normalized ? items.filter((item) => [item.title, item.slug, item.excerpt].filter(Boolean).join(" ").toLowerCase().includes(normalized)) : items;
  return <section className="mt-5 rounded-2xl border border-cyan-200 bg-cyan-50/50 p-4"><div className="flex flex-wrap items-end justify-between gap-4"><div><h3 className="font-black text-cyan-950">Find and edit a trip</h3><p className="mt-1 text-xs leading-5 text-cyan-900">Search by tour title or URL slug. You can edit its full content or change its booking visibility.</p></div><label className="w-full max-w-md text-xs font-bold text-cyan-950">Search tours<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Orange Bay, speedboat, transfer…" className="mt-1 w-full rounded-xl border border-cyan-200 bg-white px-4 py-3 text-sm font-normal text-slate-900 outline-none focus:border-cyan-600"/></label></div><div className="mt-3 flex items-center justify-between text-xs font-bold text-cyan-900"><span>{visible.length} of {items.length} trips</span>{query ? <button type="button" onClick={() => setQuery("")} className="underline">Clear search</button> : null}</div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{visible.map((item) => <article key={String(item.id)} className="rounded-xl border border-cyan-100 bg-white p-3"><span className="block truncate text-sm font-bold text-slate-900">{String(item.title)}</span><span className="mt-1 block truncate text-xs text-slate-500">/{String(item.slug)}</span><select aria-label={`Status for ${String(item.title)}`} value={String(item.listing_status || "active")} disabled={busy} onChange={(event) => void update(item, event.target.value)} className="mt-3 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold"><option value="active">Active</option><option value="paused">Paused</option><option value="unlisted">Unlisted</option></select><button type="button" onClick={() => edit(item)} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-700 px-3 py-2 text-sm font-bold text-white"><Pencil size={15}/>Edit trip</button></article>)}{!visible.length ? <p className="rounded-xl bg-white p-5 text-sm text-cyan-900">{items.length ? "No trips match this search." : "Import the current site to manage trips here."}</p> : null}</div></section>;
}
...
function ResourceForm({ resource, form, set }: { resource: Resource; form: Record<string, unknown>; set: (name: string, value: unknown) => void }) {
  ...
  if (resource === "media") return <><Input label="Public image URL" name="public_url" form={form} set={set} required/><Input label="Storage path (defaults to URL)" name="storage_path" form={form} set={set}/><div className="grid gap-3 sm:grid-cols-2"><Input label="File name" name="file_name" form={form} set={set} required/><Input label="MIME type" name="mime_type" form={form} set={set}/></div>{form.public_url ? <div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs font-bold text-slate-600">Desktop crop preview</p><div className="relative mt-1 aspect-[16/9] overflow-hidden rounded-xl bg-slate-200"><Image src={String(form.public_url)} fill unoptimized sizes="(max-width: 640px) 100vw, 420px" alt="" className="object-cover" style={{ objectPosition: `${Number(form.focal_x || 0.5) * 100}% ${Number(form.focal_y || 0.5) * 100}%` }}/></div></div><div><p className="text-xs font-bold text-slate-600">Mobile crop preview</p><div className="relative mt-1 aspect-[4/3] overflow-hidden rounded-xl bg-slate-200"><Image src={String(form.public_url)} fill unoptimized sizes="(max-width: 640px) 100vw, 420px" alt="" className="object-cover" style={{ objectPosition: `${Number(form.focal_x || 0.5) * 100}% ${Number(form.focal_y || 0.5) * 100}%` }}/></div></div></div> : null}<Area label="Accessible alt text (English fallback)" name="alt_text" form={form} set={set}/><div className="grid gap-3 sm:grid-cols-2"><Input label="Creator / photographer" name="creator" form={form} set={set}/><Input label="Source URL" name="source_url" type="url" form={form} set={set}/><Input label="License type" name="license_type" form={form} set={set}/><Input label="License URL" name="license_url" type="url" form={form} set={set}/><Select label="Rights status" name="rights_status" options={["unverified", "owned", "licensed", "open-license", "generated", "restricted"]} form={form} set={set}/><Select label="Image origin" name="authenticity" options={["unknown", "daily-red-sea", "customer", "supplier", "stock", "generated"]} form={form} set={set}/><Input label="Horizontal focal point (0–1)" name="focal_x" type="number" step="0.01" form={form} set={set}/><Input label="Vertical focal point (0–1)" name="focal_y" type="number" step="0.01" form={form} set={set}/></div><Area label="Required attribution text" name="attribution_text" form={form} set={set}/><Check label="Attribution is required" name="attribution_required" form={form} set={set}/><Input label="Legacy photo credit" name="credit" form={form} set={set}/><p className="rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900">Registering a URL does not prove publication rights. Keep status as unverified until the exact source and license have been checked. Binary upload and responsive rendition generation are not yet enabled.</p></>;
