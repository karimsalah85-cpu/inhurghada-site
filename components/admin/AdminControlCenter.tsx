...
export default function AdminControlCenter({ initialTab = "content", variant = "content" }: { initialTab?: Resource; variant?: "content" | "trips" }) {
  ...
  return <section id="control-center" className="mt-8 scroll-mt-6 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
    ...
    {data?.configured && variant === "trips" ? <TripStatusPanel items={data.content.filter((item) => item.content_type === "tour" && item.locale === "en")} busy={busy} update={updateTripStatus}/> : null}
...
function TripStatusPanel({ items, busy, update }: { items: RecordValue[]; busy: boolean; update: (item: RecordValue, status: string) => Promise<void> }) {
  return <section className="mt-5 rounded-2xl border border-cyan-200 bg-cyan-50/50 p-4"><h3 className="font-black text-cyan-950">Trip visibility & booking</h3><p className="mt-1 text-xs leading-5 text-cyan-900">Active trips are visible and bookable. Paused trips stay visible but cannot be booked. Unlisted trips are hidden from public pages and the sitemap.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => <label key={String(item.id)} className="rounded-xl border border-cyan-100 bg-white p-3"><span className="block truncate text-sm font-bold text-slate-900">{String(item.title)}</span><span className="mt-1 block truncate text-xs text-slate-500">/{String(item.slug)}</span><select aria-label={`Status for ${String(item.title)}`} value={String(item.listing_status || "active")} disabled={busy} onChange={(event) => void update(item, event.target.value)} className="mt-3 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold"><option value="active">Active</option><option value="paused">Paused</option><option value="unlisted">Unlisted</option></select></label>)}{!items.length ? <p className="text-sm text-cyan-900">Import the current site to manage trip statuses here.</p> : null}</div></section>;
}
