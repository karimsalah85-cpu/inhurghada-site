...
export default function AdminDashboard({ initialBookings, initialExpenses, initialSuppliers, initialSalesPeople, migrationPending = false }: { initialBookings: Booking[]; initialExpenses: Expense[]; initialSuppliers: Supplier[]; initialSalesPeople: SalesPerson[]; migrationPending?: boolean }) {
  ...
  async function sendStatusEmail(booking: Booking) {
    ...
    if (!window.confirm(`Resend ${booking.customer_email} the current booking and payment status email?\n\nBooking: ${booking.status}\nPayment: ${booking.payment_status}`)) return;
  ...
  async function addPartner(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusyId("partner"); setError("");
    try {
      const result = await api<{ partner: Supplier | SalesPerson; type: PartnerType }>("/api/admin/partners", { method: "POST", body: JSON.stringify({ ...partner, type: partnerType }) });
      if (result.type === "supplier") setSuppliers((items) => [...items, result.partner as Supplier].sort((a, b) => a.name.localeCompare(b.name)));
      else setSalesPeople((items) => [...items, result.partner as SalesPerson].sort((a, b) => a.name.localeCompare(b.name)));
      setPartner({ name: "", contact_name: "", phone: "", email: "", commission_percent: "", notes: "" });
      feedback(`${partnerType === "supplier" ? "Supplier" : "Sales person"} created.`);
      router.refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not create this record."); }
    finally { setBusyId(null); }
  }

  async function deletePartner(type: PartnerType, id: string, name: string) {
    if (!window.confirm(`Delete ${name}? Existing expense records will be kept.`)) return;
    setBusyId(`partner-${id}`); setError("");
    try {
      await api(`/api/admin/partners/${type}/${id}`, { method: "DELETE" });
      if (type === "supplier") setSuppliers((items) => items.filter((item) => item.id !== id));
    ...
    <div className="mt-10 grid gap-8 xl:grid-cols-[1.7fr_0.8fr]">
      ...
      <aside id="expenses" className="h-fit scroll-mt-6 rounded-3xl bg-white p-6 shadow-sm">
        ...
        <form className="mt-6 space-y-4" onSubmit={addExpense}>
          <label className="block text-sm font-semibold">Expense type<select value={expense.expense_type} onChange={(event) => setExpense({ ...expense, expense_type: event.target.value, description: expenseOptions.find(([value]) => value === event.target.value)?.[1] || expense.description })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-3 font-normal">{expenseOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="block text-sm font-semibold">Description<input required maxLength={200} value={expense.description} onChange={(event) => setExpense({ ...expense, description: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" placeholder="What was paid for?" /></label>
          <label className="block text-sm font-semibold">Amount (USD)<input required type="number" min="0.01" max="1000000" step="0.01" value={expense.amount} onChange={(event) => setExpense({ ...expense, amount: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" placeholder="0.00" /></label>
          {expense.expense_type === "supplier_per_trip" ? <label className="block text-sm font-semibold">Supplier<select required value={expense.supplier_id} onChange={(event) => setExpense({ ...expense, supplier_id: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-3 font-normal"><option value="">Choose supplier</option>{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : null}
          {expense.expense_type === "sales_commission" ? <label className="block text-sm font-semibold">Sales person<select required value={expense.sales_person_id} onChange={(event) => setExpense({ ...expense, sales_person_id: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-3 font-normal"><option value="">Choose sales person</option>{salesPeople.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : null}
          {["supplier_per_trip", "sales_commission"].includes(expense.expense_type) ? <label className="block text-sm font-semibold">Booking <span className="font-normal text-slate-400">optional</span><select value={expense.booking_id} onChange={(event) => setExpense({ ...expense, booking_id: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-3 font-normal"><option value="">No booking selected</option>{bookings.filter((item) => item.status !== "cancelled").map((item) => <option key={item.id} value={item.id}>{item.reference} · {item.tour_name || "Transfer"}</option>)}</select></label> : null}
          <label className="block text-sm font-semibold">Category <span className="font-normal text-slate-400">optional</span><input maxLength={80} value={expense.category} onChange={(event) => setExpense({ ...expense, category: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" placeholder="Internal label" /></label>
          <label className="block text-sm font-semibold">Date<input required type="date" value={expense.date} onChange={(event) => setExpense({ ...expense, date: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label>
    ...
    <section id="partners" className="mt-8 scroll-mt-6 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <form onSubmit={addPartner} className="space-y-4 rounded-2xl bg-slate-50 p-4">
          <label className="block text-sm font-semibold">Record type<select value={partnerType} onChange={(event) => setPartnerType(event.target.value as PartnerType)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-3 font-normal"><option value="supplier">Supplier</option><option value="sales_person">Sales person</option></select></label>
          <label className="block text-sm font-semibold">Name<input required maxLength={120} value={partner.name} onChange={(event) => setPartner({ ...partner, name: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label>
          {partnerType === "supplier" ? <label className="block text-sm font-semibold">Contact person<input maxLength={120} value={partner.contact_name} onChange={(event) => setPartner({ ...partner, contact_name: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label> : <label className="block text-sm font-semibold">Commission % <span className="font-normal text-slate-400">optional</span><input type="number" min="0" max="100" step="0.01" value={partner.commission_percent} onChange={(event) => setPartner({ ...partner, commission_percent: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label>}
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Phone<input maxLength={40} value={partner.phone} onChange={(event) => setPartner({ ...partner, phone: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="block text-sm font-semibold">Email<input type="email" maxLength={160} value={partner.email} onChange={(event) => setPartner({ ...partner, email: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div>
          <label className="block text-sm font-semibold">Notes <span className="font-normal text-slate-400">optional</span><textarea maxLength={500} value={partner.notes} onChange={(event) => setPartner({ ...partner, notes: event.target.value })} className="mt-1 min-h-24 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label>
          <button disabled={busyId === "partner"} className="w-full rounded-xl bg-cyan-700 py-3 font-bold text-white hover:bg-cyan-800 disabled:opacity-60">{busyId === "partner" ? "Saving…" : `Create ${partnerType === "supplier" ? "supplier" : "sales person"}`}</button>
        </form>
        <div className="grid gap-6 md:grid-cols-2">
          <PartnerList title="Suppliers" empty="No suppliers yet." items={suppliers.map((item) => ({ id: item.id, name: item.name, detail: item.contact_name || item.phone || item.email || "No contact details" }))} busyId={busyId} onDelete={(id, name) => deletePartner("supplier", id, name)} />
          <PartnerList title="Sales people" empty="No sales people yet." items={salesPeople.map((item) => ({ id: item.id, name: item.name, detail: item.commission_percent != null ? `${item.commission_percent}% commission` : item.phone || item.email || "No commission set" }))} busyId={busyId} onDelete={(id, name) => deletePartner("sales_person", id, name)} />
        </div>
      </div>
...
function SalesPerformancePanel({salesPeople,bookings,expenses}:{salesPeople:SalesPerson[];bookings:Booking[];expenses:Expense[]}) {
  const rows=salesPeople.map(person=>{
    const sales=bookings.filter(booking=>booking.sales_person_id===person.id&&booking.status!=="cancelled"&&booking.payment_status!=="refunded");
    const total=sales.reduce((sum,booking)=>sum+Number(booking.amount||0),0);
    const earned=sales.reduce((sum,booking)=>sum+Number(booking.amount||0)*Number(booking.sales_commission_percent??person.commission_percent??0)/100,0);
    const paid=expenses.filter(item=>item.expense_type==="sales_commission"&&item.sales_person_id===person.id).reduce((sum,item)=>sum+Number(item.amount||0),0);
    return{person,sales,total,earned,paid,outstanding:Math.max(0,earned-paid)};
  });
  return <div className="mt-8 border-t pt-6"><div><h3 className="text-xl font-black">Sales performance</h3><p className="mt-1 text-sm text-slate-500">Active assigned bookings and commission based on the rate saved when each booking is assigned. Record commission payments in Expenses.</p></div><div className="mt-4 overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Sales person</th><th className="p-3">Sales</th><th className="p-3">Bookings</th><th className="p-3">Commission earned</th><th className="p-3">Paid</th><th className="p-3">Outstanding</th></tr></thead><tbody>{rows.map(({person,sales,total,earned,paid,outstanding})=><tr key={person.id} className="border-t"><td className="p-3"><strong>{person.name}</strong><br/><span className="text-xs text-slate-500">Default {Number(person.commission_percent||0)}%</span></td><td className="p-3 font-bold">{money(total)}</td><td className="p-3">{sales.length}</td><td className="p-3">{money(earned)}</td><td className="p-3 text-emerald-700">{money(paid)}</td><td className="p-3 font-bold text-amber-700">{money(outstanding)}</td></tr>)}</tbody></table>{!rows.length?<p className="p-6 text-center text-slate-500">Create a sales person to begin tracking performance.</p>:null}</div></div>;
}
...
function PartnerList({ title, empty, items, busyId, onDelete }: { title: string; empty: string; items: { id: string; name: string; detail: string }[]; busyId: string | null; onDelete: (id: string, name: string) => void }) {
  return <div><div className="flex items-center justify-between"><h3 className="font-black text-slate-900">{title}</h3><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500">{items.length}</span></div>{items.length ? <div className="mt-3 space-y-3">{items.map((item) => <article key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-slate-200 p-3"><div><p className="font-bold text-slate-900">{item.name}</p><p className="mt-1 text-xs text-slate-500">{item.detail}</p></div><button type="button" aria-label={`Delete ${item.name}`} onClick={() => onDelete(item.id, item.name)} disabled={busyId === `partner-${item.id}`} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-700 disabled:opacity-40"><Trash2 size={16}/></button></article>)}</div> : <p className="mt-3 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">{empty}</p>}</div>;
}
