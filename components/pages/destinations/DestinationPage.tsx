...
export async function DestinationPage({ params, locale = "en" }: Props & { locale?: Locale }) {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="min-h-screen bg-surface-muted pb-20 pt-24"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, "\\u003c") }}/><section className="relative overflow-hidden px-6 py-8 text-white sm:px-8 sm:py-14"><Image src={destination.image} alt={copy.imageAlt} fill sizes="100vw" priority className="object-cover"/><div className="absolute inset-0 bg-ink/70"/><div className="relative mx-auto max-w-7xl"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-ocean-soft sm:text-sm">{copy.eyebrow}</p><h1 className="mt-2 text-3xl font-black sm:mt-3 sm:text-5xl">{copy.heading}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-line sm:mt-4 sm:text-lg sm:leading-8">{copy.description}</p></div></section><div className="mx-auto max-w-7xl px-6 sm:px-8"><section className="pt-5 sm:pt-8"><h2 className="text-xl font-black text-ink sm:text-3xl">{copy.bookable}</h2><div className="mt-3 sm:mt-5"><CategoryTourExplorer tours={destinationTours} locale={locale}/></div></section><section className="pt-14">{categories.length ? <h2 className="text-3xl font-black text-ink">{copy.categories}</h2> : null}<p className="mt-3 text-muted">{copy.trips(destinationTours.length)}</p>{categories.length ? <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map((category) => <Link key={category.slug} href={localePath(locale, `/${destination.slug}/${category.slug}`)} className="rounded-3xl border border-line bg-white p-6 shadow-sm transition hover:border-ocean"><p className="text-sm font-bold uppercase tracking-[0.2em] text-ocean-dark">{category.eyebrow}</p><h3 className="mt-3 text-2xl font-black text-ink">{category.title}</h3><p className="mt-3 text-muted">{destinationTours.filter(category.matches).length} {copy.experiences}</p></Link>)}</div> : null}</section>{otherDestinations.length ? <section className="mt-12 rounded-[2rem] bg-ink p-8 text-white"><p className="font-bold uppercase tracking-[0.2em] text-ocean-soft">{copy.otherDestinations}</p><div className="mt-5 flex flex-wrap gap-3">{otherDestinations.map((item) => <Link key={item.slug} href={localePath(locale, `/destinations/${item.slug}`)} className="rounded-full bg-white px-5 py-3 font-bold text-ink">{item.name} →</Link>)}</div></section> : null}</div></main>;
}
