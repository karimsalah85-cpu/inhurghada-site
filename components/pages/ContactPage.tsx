  ...
  return (
    <main className="min-h-screen bg-surface-muted px-6 pb-20 pt-32 sm:px-8">
      <div className="mx-auto max-w-5xl">
        <p className="font-semibold uppercase tracking-[0.28em] text-ocean-dark">{tr("We are here to help", "Wir helfen gerne", "Мы готовы помочь")}</p>
        <h1 className="mt-4 text-4xl font-black text-ink sm:text-6xl">{tr("Contact Daily Red Sea", "Daily Red Sea kontaktieren", "Связаться с Daily Red Sea")}</h1>
        <p className="mt-6 max-w-3xl text-lg leading-8 text-muted">{tr("For the fastest response, send your travel date, hotel, number of adults and children, and the tour or transfer you are considering.", "Für eine schnelle Antwort sende uns Reisedatum, Hotel, Anzahl der Erwachsenen und Kinder sowie den gewünschten Ausflug oder Transfer.", "Чтобы получить быстрый ответ, сообщите дату поездки, отель, количество взрослых и детей, а также интересующую экскурсию или трансфер.")}</p>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          <TrackedExternalLink event="whatsapp_click" eventData={{ location: "contact_page" }} href={whatsappUrl(tr("Hello Daily Red Sea, I need help with a booking.", "Hallo Daily Red Sea, ich brauche Hilfe bei einer Buchung.", "Здравствуйте! Мне нужна помощь с бронированием."))} target="_blank" rel="noopener noreferrer" className="rounded-3xl border border-emerald-200 bg-emerald-50 p-7 transition hover:-translate-y-1"><p className="text-sm font-bold uppercase tracking-wider text-emerald-700">{tr("Fastest", "Am schnellsten", "Самый быстрый способ")}</p><h2 className="mt-3 text-2xl font-black text-ink">WhatsApp</h2><p className="mt-3 text-muted">{tr("Tour advice, availability, pickup changes, and booking help.", "Beratung, Verfügbarkeit, Änderungen der Abholung und Buchungshilfe.", "Советы по экскурсиям, наличие мест, изменение трансфера и помощь с бронированием.")}</p></TrackedExternalLink>
          <TrackedExternalLink event="email_click" eventData={{ location: "contact_page" }} href={`mailto:${contactEmail}`} className="rounded-3xl border border-line bg-white p-7 transition hover:-translate-y-1"><p className="text-sm font-bold uppercase tracking-wider text-ocean-dark">E-Mail</p><h2 className="mt-3 text-xl font-black text-ink">{contactEmail}</h2><p className="mt-3 text-muted">{tr("Useful for detailed requests and documents.", "Geeignet für ausführliche Anfragen und Dokumente.", "Подходит для подробных запросов и документов.")}</p></TrackedExternalLink>
          <TrackedExternalLink event="phone_click" eventData={{ location: "contact_page" }} href={`tel:${displayPhoneNumber.replace(/\s/g, "")}`} className="rounded-3xl border border-line bg-white p-7 transition hover:-translate-y-1"><p className="text-sm font-bold uppercase tracking-wider text-ocean-dark">{tr("Telephone", "Telefon", "Телефон")}</p><h2 className="mt-3 text-xl font-black text-ink">{displayPhoneNumber}</h2><p className="mt-3 text-muted">{tr("For urgent same-day assistance.", "Für dringende Hilfe am selben Tag.", "Для срочной помощи в день поездки.")}</p></TrackedExternalLink>
        </div>
        <section className="mt-10 rounded-3xl border border-line bg-white p-8"><h2 className="text-2xl font-black text-ink">{tr("Already booked?", "Bereits gebucht?", "Уже забронировали?")}</h2><p className="mt-4 leading-7 text-muted">{tr("Include your booking reference when contacting us. For pickup changes or urgent questions close to departure, use WhatsApp so the team can identify your booking quickly.", "Nenne bei der Kontaktaufnahme deine Buchungsnummer. Für Änderungen der Abholung oder dringende Fragen kurz vor Abfahrt nutze WhatsApp.", "При обращении укажите номер бронирования. Для изменения трансфера или срочных вопросов перед отправлением используйте WhatsApp.")}</p><Link href={localePath(locale, "/booking")} className="mt-6 inline-flex rounded-full bg-ocean-dark px-6 py-3 font-bold text-white">{tr("Find my booking", "Buchung finden", "Найти бронирование")}</Link></section>
      </div>
    </main>
  );
}
