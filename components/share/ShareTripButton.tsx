...
import { buildTripShareUrl, tripShareText, whatsappShareUrl } from "@/lib/share-trip";
import { trackEvent } from "@/lib/analytics";

export default function ShareTripButton({ locale, tourSlug, compact = false }: { locale: Locale; tourSlug: string; compact?: boolean }) {
  const params = useSearchParams();
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const ar = locale === "ar";
  const url = typeof window === "undefined" ? "" : buildTripShareUrl(window.location.origin, locale, tourSlug, params.get("date") || undefined);
  const text = tripShareText(locale);
  async function nativeShare() {
    trackEvent("share_opened", { item_id: tourSlug, destination: "jeddah" });
    if (navigator.share) {
      try { await navigator.share({ title: ar ? "رحلة غروب جدة" : "Jeddah sunset cruise", text, url }); trackEvent("share_completed", { method: "native", item_id: tourSlug }); return; } catch (error) { if ((error as DOMException).name === "AbortError") return; }
    }
    setOpen((value) => !value);
  }
  async function copy() {
    try { await navigator.clipboard.writeText(url); setCopied(true); trackEvent("share_completed", { method: "copy", item_id: tourSlug }); } catch { setCopied(false); }
  }
  return <div className="relative inline-flex">
    ...
    {open ? <div className="absolute end-0 top-full z-40 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 text-sm shadow-xl" role="menu">
      ...
      <a href={whatsappShareUrl(text, url)} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("share_completed", { method: "whatsapp", item_id: tourSlug })} className="flex items-center gap-2 rounded-xl px-3 py-2.5 hover:bg-slate-50" role="menuitem">WhatsApp</a>
      <a href={`mailto:?subject=${encodeURIComponent(ar ? "رحلة غروب جدة" : "Jeddah sunset cruise")}&body=${encodeURIComponent(`${text}\n\n${url}`)}`} className="flex items-center gap-2 rounded-xl px-3 py-2.5 hover:bg-slate-50" role="menuitem"><Mail size={17}/>{ar ? "البريد الإلكتروني" : "Email"}</a>
    </div> : null}
  </div>;
}
