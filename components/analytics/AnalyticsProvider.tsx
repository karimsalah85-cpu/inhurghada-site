...
function CookieBanner({ onSave }: { onSave: (preferences: ConsentPreferences) => void }) {
  ...
  return <section role="region" aria-labelledby="cookie-preferences-title" dir={language === "ar" ? "rtl" : "ltr"} className="fixed inset-x-4 bottom-[calc(4.75rem+1rem+env(safe-area-inset-bottom))] z-[100] mx-auto max-w-3xl rounded-3xl border border-line bg-white p-5 shadow-2xl sm:p-6 xl:bottom-4"><h2 id="cookie-preferences-title" className="text-lg font-black text-ink">{copy.cookieTitle}</h2><p className="mt-2 text-sm leading-6 text-muted">{copy.cookieText}</p><fieldset className="mt-4 flex flex-wrap gap-4 text-sm font-semibold text-ink"><legend className="sr-only">{copy.optionalCookies}</legend><label className="flex items-center gap-2"><input type="checkbox" checked={analytics} onChange={(event) => setAnalytics(event.target.checked)} /> {copy.analytics}</label><label className="flex items-center gap-2"><input type="checkbox" checked={marketing} onChange={(event) => setMarketing(event.target.checked)} /> {copy.marketing}</label></fieldset><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => onSave({ analytics: false, marketing: false })} className="rounded-full border border-line px-4 py-2 text-sm font-bold">{copy.reject}</button><button type="button" onClick={() => onSave({ analytics, marketing })} className="rounded-full border border-ocean-dark px-4 py-2 text-sm font-bold text-ocean-dark">{copy.save}</button><button type="button" onClick={() => onSave({ analytics: true, marketing: true })} className="rounded-full bg-ocean-dark px-4 py-2 text-sm font-bold text-white">{copy.accept}</button></div></section>;
}
