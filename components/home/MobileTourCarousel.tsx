  ...
  return (
    <div aria-label={label} aria-roledescription="carousel">
      ...
      {items.length > 1 ? <div className="mt-3 flex items-center justify-center gap-4 md:hidden">
        ...
        <div className="flex flex-wrap justify-center gap-0.5" aria-label={`Tour ${activeIndex + 1} of ${items.length}`}>
          {items.map((_, index) => (
            <button key={index} type="button" onClick={() => goTo(index)} aria-label={`Go to tour ${index + 1}`} aria-current={index === activeIndex ? "true" : undefined} className="grid size-6 place-items-center rounded-full">
              <span className={`block size-2.5 rounded-full transition ${index === activeIndex ? "bg-ocean-dark" : "bg-line"}`} />
            </button>
          ))}
        </div>
