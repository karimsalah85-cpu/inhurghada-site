...
export default function Home({ initialTours }: { initialTours: Tour[] }) {
  return (
    <Suspense fallback={<main className="min-h-screen bg-white" />}>
      <HomeContent initialTours={initialTours} />
    </Suspense>
  );
}
...
function HomeContent({ initialTours }: { initialTours: Tour[] }) {
  ...
  const [urlQuery, setUrlQuery] = useState<{ search: string; date: string | null; guests: string | null }>({ search: "", date: null, guests: null });
  useEffect(() => {
  ...
  const destinationDetails: Record<string, { signature: string; startingPrice: string }> = {
    ...
    "el-gouna": { signature: tr("Beach & desert horse riding", "Reiten am Strand & in der Wüste", "Конные прогулки по пляжу и пустыне", "ركوب الخيل على الشاطئ وفي الصحراء", "Jazda konna po plaży i pustyni", "海滩与沙漠骑马"), startingPrice: "€45" },
  ...
  const destinationLocaleCopy: Record<string, { country: string; region: string; tagline: string }> = {
    ...
    "el-gouna": {
      country: tr("Egypt", "Ägypten", "Египет", "مصر", "Egipt", "埃及"),
      region: tr("Red Sea Governorate", "Gouvernement Rotes Meer", "Мухафаза Красное море", "محافظة البحر الأحمر", "Muhafaza Morza Czerwonego", "红海省"),
      tagline: tr("Lagoons, beaches and relaxed Red Sea activities", "Lagunen, Strände und entspannte Aktivitäten am Roten Meer", "Лагуны, пляжи и спокойный отдых на Красном море", "بحيرات وشواطئ وأنشطة هادئة على البحر الأحمر", "Laguny, plaże i spokojne atrakcje nad Morzem Czerwonym", "潟湖、海滩与悠闲的红海体验"),
    },
    jeddah: {
  ...
  return (
    ...
    <>
      ...
      <ContinuePlanningRail tours={displayTours} />
