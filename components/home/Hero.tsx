...
export default function Hero() {
  ...
  const planner = (
    <div className="grid gap-2.5 md:grid-cols-[1.25fr_1fr_.7fr_auto] md:items-stretch">
      <PlannerField icon={MapPin} label={t("destination")}>
        <select value={destination} onChange={(event) => setDestination(event.target.value)} aria-label={t("destination")} className="w-full bg-transparent text-sm font-semibold text-slate-900 outline-none">
      ...
      <PlannerField icon={Calendar} label={t("travelDate")}>
        <input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="w-full bg-transparent text-sm font-semibold text-slate-900 outline-none" />
      </PlannerField>
      <PlannerField icon={Users} label={t("guests")}>
        <input type="number" min="1" value={guests} onChange={(event) => setGuests(event.target.value)} className="w-full bg-transparent text-sm font-semibold text-slate-900 outline-none" />
      </PlannerField>
      <button type="button" onClick={searchTours} className="flex min-h-16 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-700 px-6 font-bold text-white shadow-lg shadow-cyan-900/20 transition hover:-translate-y-0.5 hover:from-cyan-500 hover:to-blue-800 focus-visible:ring-4 focus-visible:ring-cyan-300">
  ...
  return (
    <section className="relative min-h-[720px] overflow-hidden sm:min-h-[700px]">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-slate-950/95 via-slate-950/65 to-slate-950/10 sm:via-slate-950/55" />
      ...
      <div className="relative z-10 mx-auto flex min-h-[720px] max-w-7xl items-center px-5 pb-24 pt-32 sm:min-h-[700px] sm:px-8 sm:pb-28 sm:pt-36">
        <div className="w-full max-w-5xl text-white">
          ...
          <p className="text-xs font-black uppercase tracking-[0.28em] text-cyan-300 sm:text-sm">{t("discoverHurghada")}</p>
          <h1 className="mt-4 max-w-4xl text-4xl font-black leading-[1.02] tracking-tight sm:text-6xl lg:text-7xl">{t("heroTitle")}</h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-100 sm:text-lg sm:leading-8">{t("heroDescription")}</p>
          ...
          <details className="group mt-8 rounded-3xl border border-white/25 bg-white/95 p-3 text-slate-950 shadow-2xl backdrop-blur md:hidden" open>
...
function PlannerField({ icon: Icon, label, children }: { icon: typeof MapPin; label: string; children: React.ReactNode }) {
  return (
    <label className="flex min-h-16 items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 transition focus-within:border-cyan-500 focus-within:bg-white">
      <Icon className="shrink-0 text-cyan-700" size={20} />
