...
const copy = {
  en: {
    ...
    detail: "Hurghada, Marsa Alam and El Gouna sit on Egypt’s Red Sea coast, and Jeddah opens up the Saudi side of the same sea, with more destinations to come. Compare destination, duration, inclusions, pickup details and transparent prices before you book.",
    items: [
  ...
  de: {
    ...
    detail: "Hurghada, Marsa Alam und El Gouna liegen an der ägyptischen Küste des Roten Meeres, Jeddah erschließt die saudische Seite desselben Meeres – weitere Ziele folgen. Vergleiche Reiseziel, Dauer, Leistungen, Abholdetails und transparente Preise vor der Buchung.",
    items: [
  ...
  ru: {
    ...
    detail: "Хургада, Марса-Алам и Эль-Гуна расположены на египетском побережье Красного моря, а Джидда открывает саудовскую сторону того же моря; новые направления появятся позже. Перед бронированием сравните направление, продолжительность, услуги, детали трансфера и прозрачные цены.",
    items: [
  ...
  ar: {
    ...
    detail: "تقع الغردقة ومرسى علم والجونة على الساحل المصري للبحر الأحمر، وتفتح جدة الجانب السعودي من البحر نفسه، مع وجهات أخرى قادمة لاحقًا. قارن بين الوجهة والمدة والخدمات المشمولة وتفاصيل الاستلام والأسعار الواضحة قبل الحجز.",
    items: [
  ...
  pl: {
    ...
    detail: "Hurghada, Marsa Alam i El Gouna leżą na egipskim wybrzeżu Morza Czerwonego, a Dżudda otwiera saudyjską stronę tego samego morza; kolejne kierunki wkrótce. Przed rezerwacją porównaj kierunek, czas trwania, świadczenia, szczegóły odbioru i przejrzyste ceny.",
    items: [
  ...
  zh: {
    ...
    detail: "赫尔格达、马萨阿拉姆和埃尔古纳位于埃及的红海海岸，吉达则通向同一片海的沙特一侧，未来还会有更多目的地。预订前请比较目的地、时长、包含项目、接送详情和透明价格。",
    items: [
...
export default function HurghadaTravelGuide() {
  ...
  return <section aria-labelledby="hurghada-guide-title" className="bg-white px-6 py-20 sm:px-8"><div className="mx-auto max-w-7xl"><div className="max-w-3xl"><p className="font-semibold uppercase tracking-[0.24em] text-ocean-dark">{content.eyebrow}</p><h2 id="hurghada-guide-title" className="mt-3 text-4xl font-black text-ink">{content.title}</h2><p className="mt-5 text-lg leading-8 text-muted">{content.intro}</p><p className="mt-4 leading-8 text-muted">{content.detail}</p></div><div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{content.items.map(([title, text], index) => { const Icon = icons[index]; return <article key={title} className="rounded-3xl border border-line bg-surface-muted p-6"><Icon aria-hidden="true" className="text-ocean-dark" size={28}/><h3 className="mt-5 text-xl font-bold text-ink">{title}</h3><p className="mt-3 leading-7 text-muted">{text}</p></article>; })}</div></div></section>;
}
