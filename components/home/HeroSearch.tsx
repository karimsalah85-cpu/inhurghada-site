...
import { MapPin, Search, ArrowRight, LayoutGrid, X } from "lucide-react";
...
const copy: Record<Locale, { placeholder: string; button: string; destinations: string; thingsToDo: string; popular: string; seeAll: (query: string) => string; noMatch: string; tours: string; from: string; label: string; clear: string; categories: string }> = {
  en: { placeholder: "Find places and things to do", button: "Search", destinations: "Destinations", thingsToDo: "Things to do", popular: "Popular right now", seeAll: (q) => `See all results for “${q}”`, noMatch: "No exact match. Browse all tours", tours: "tours", from: "from", clear: "Clear search", categories: "Categories", label: "Search tours and destinations" },
  de: { placeholder: "Orte und Aktivitäten finden", button: "Suchen", destinations: "Reiseziele", thingsToDo: "Aktivitäten", popular: "Gerade beliebt", seeAll: (q) => `Alle Ergebnisse für „${q}“`, noMatch: "Kein genauer Treffer. Alle Ausflüge ansehen", tours: "Ausflüge", from: "ab", clear: "Suche löschen", categories: "Kategorien", label: "Ausflüge und Reiseziele suchen" },
  ru: { placeholder: "Найдите места и развлечения", button: "Найти", destinations: "Направления", thingsToDo: "Экскурсии", popular: "Популярно сейчас", seeAll: (q) => `Все результаты по запросу «${q}»`, noMatch: "Точных совпадений нет. Смотреть все экскурсии", tours: "экскурсий", from: "от", clear: "Очистить поиск", categories: "Категории", label: "Поиск экскурсий и направлений" },
  ar: { placeholder: "ابحث عن أماكن وأنشطة", button: "بحث", destinations: "الوجهات", thingsToDo: "الأنشطة", popular: "الأكثر طلبًا الآن", seeAll: (q) => `عرض كل النتائج لـ «${q}»`, noMatch: "لا توجد نتيجة مطابقة. تصفح كل الرحلات", tours: "رحلات", from: "من", clear: "مسح البحث", categories: "الفئات", label: "ابحث عن الرحلات والوجهات" },
  pl: { placeholder: "Znajdź miejsca i atrakcje", button: "Szukaj", destinations: "Kierunki", thingsToDo: "Atrakcje", popular: "Teraz popularne", seeAll: (q) => `Wszystkie wyniki dla „${q}”`, noMatch: "Brak dokładnych wyników. Zobacz wszystkie wycieczki", tours: "wycieczek", from: "od", clear: "Wyczyść wyszukiwanie", categories: "Kategorie", label: "Szukaj wycieczek i kierunków" },
  zh: { placeholder: "搜索地点和活动", button: "搜索", destinations: "目的地", thingsToDo: "活动", popular: "当前热门", seeAll: (q) => `查看“${q}”的全部结果`, noMatch: "没有完全匹配的结果，浏览全部行程", tours: "个行程", from: "起", clear: "清除搜索", categories: "类别", label: "搜索行程和目的地" },
};
...
type Option =
  ...
  | { kind: "category"; key: string; href: string; name: string; meta: string }
...
export default function HeroSearch({ tours, className = "" }: { tours: Tour[]; className?: string }) {
  ...
  const { destinationOptions, categoryOptions, tourOptions, allOption } = useMemo(() => {
    ...
    // Categories, like GetYourGuide's "Day trips · 100+ activities": a category
    // shows when its own name matches, or when it holds 2+ of the matching tours.
    const categoryRows: Option[] = [];
    if (q) {
      const matched = filterTours(tours, trimmed);
      const groups = new Map<string, { category: string; destination: string; count: number; nameMatches: boolean }>();
      for (const tour of tours) {
        if (!tour.category) continue;
        const destination = tour.destinationSlug || "hurghada";
        const key = `${destination}|${tour.category}`;
        const group = groups.get(key) ?? { category: tour.category, destination, count: 0, nameMatches: normalize(tour.category).includes(q) };
        group.count += 1;
        groups.set(key, group);
      }
      const matchedPerGroup = new Map<string, number>();
      for (const tour of matched) if (tour.category) {
        const key = `${tour.destinationSlug || "hurghada"}|${tour.category}`;
        matchedPerGroup.set(key, (matchedPerGroup.get(key) ?? 0) + 1);
      }
      [...groups.entries()]
        ...
        .forEach(([key, group]) => {
          ...
          categoryRows.push({
            kind: "category",
            key: `c-${key}`,
            href: `${localePath(language, "/tours")}?destination=${group.destination}&search=${encodeURIComponent(group.category)}`,
            name: group.category,
            meta: `${group.count} ${text.tours} · ${destinationName}`,
          });
        });
    }
    ...
    const all: Option[] = q
      ...
      : [];
    return { destinationOptions: destinationMatches, categoryOptions: categoryRows, tourOptions: tourOpts, allOption: all };
  }, [language, text, tours, trimmed]);

  const options = [...destinationOptions, ...categoryOptions, ...tourOptions, ...allOption];
    ...
    <div className={`relative ${className}`}>
      ...
      >
        ...
        {query ? (
          <button
            type="button"
            onClick={() => { setQuery(""); setActive(-1); setOpen(true); inputRef.current?.focus(); }}
            aria-label={text.clear}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-muted transition hover:bg-surface-muted hover:text-ink focus-visible:ring-2 focus-visible:ring-ocean"
          >
      ...
      {open && options.length ? (
        <div className="absolute inset-x-0 top-full z-40 mt-2 max-h-[min(70vh,34rem)] overflow-y-auto rounded-3xl border border-line bg-white p-2 text-ink shadow-2xl">
          <ul id={listId} role="listbox" aria-label={text.label} className="grid gap-0.5">
            ...
            {categoryOptions.length ? <li role="presentation" className="px-3 pb-1 pt-3 text-[11px] font-black uppercase tracking-[0.16em] text-muted">{text.categories}</li> : null}
            {categoryOptions.map((option) => option.kind === "category" && row(option, (
              <>
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface-muted text-ink"><LayoutGrid size={18} aria-hidden="true" /></span>
                <span className="min-w-0 flex-1"><span className="block truncate font-bold">{option.name}</span><span className="block truncate text-xs text-muted">{option.meta}</span></span>
              </>
            )))}
