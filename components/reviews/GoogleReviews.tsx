...
export default function GoogleReviews({ className = "" }: { className?: string }) {
  ...
  return <section className={`rounded-[2rem] border border-blue-100 bg-white p-7 shadow-sm sm:p-10 ${className}`}>
    <div className="flex flex-wrap items-end justify-between gap-5">
      <div><p className="font-bold uppercase tracking-[0.24em] text-blue-600">{labels.eyebrow}</p><h2 className="mt-3 text-3xl font-black text-slate-950 sm:text-4xl">{labels.title}</h2></div>
      {data ? <a href={data.placeUri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-slate-200 px-5 py-3 font-black text-slate-950 hover:bg-slate-50"><span className="text-2xl text-amber-500">★</span><span>{data.rating.toFixed(1)} <small className="block font-medium text-slate-500">{data.reviewCount.toLocaleString()} reviews</small></span></a> : null}
    </div>

    {loading ? <div className="mt-8" role="status" aria-live="polite"><p className="text-sm font-semibold text-slate-500">{labels.loading}</p><div className="mt-4 grid gap-4 md:grid-cols-3" aria-hidden="true">{[0,1,2].map((item) => <div key={item} className="h-52 animate-pulse rounded-2xl bg-slate-100" />)}</div></div> : null}
    {!loading && data?.reviews.length ? <div className="mt-8 grid gap-5 lg:grid-cols-3">{data.reviews.slice(0, 3).map((review, index) => <article key={`${review.authorName}-${index}`} className="flex min-h-64 flex-col rounded-2xl border border-slate-200 bg-slate-50 p-5">
      <div className="flex items-center justify-between gap-3"><a href={review.authorUri || data.placeUri} target="_blank" rel="noopener noreferrer" className="font-black text-slate-950 hover:text-blue-700">{review.authorName}</a><div className="flex" aria-label={`${review.rating} out of 5 stars`}>{Array.from({ length: 5 }, (_, star) => <Star key={star} size={15} className={star < review.rating ? "fill-amber-400 text-amber-400" : "text-slate-300"}/>)}</div></div>
      <p className="mt-4 line-clamp-6 flex-1 text-sm leading-6 text-slate-700">{review.text}</p>
      <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-slate-200 pt-4 text-xs"><span className="text-slate-500">{review.relativeTime}</span>{review.reviewUri ? <a href={review.reviewUri} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 font-bold text-blue-700">{labels.view}<ExternalLink size={12}/></a> : null}{review.reportUri ? <a href={review.reportUri} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-slate-500"><Flag size={11}/>{labels.report}</a> : null}</div>
    </article>)}</div> : null}

    {!loading && (failed || !data?.reviews.length) ? <div className="mt-8 rounded-2xl bg-blue-50 p-6 text-slate-700">Google review content is available on our official Google profile.</div> : null}
    <div className="mt-8 flex flex-wrap items-center justify-between gap-5 border-t border-slate-100 pt-6">
      <p className="max-w-2xl text-xs leading-5 text-slate-500"><strong className="text-slate-700">Google Maps</strong> · {labels.ordered} {labels.policy}</p>
      <div className="flex flex-wrap gap-3"><a href={data?.placeUri || googleMapsUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-blue-700 px-5 py-2.5 text-sm font-black text-white hover:bg-blue-800">{labels.read}</a><a href={googleReviewUrl} target="_blank" rel="noopener noreferrer" className="rounded-full border border-blue-200 px-5 py-2.5 text-sm font-black text-blue-800 hover:bg-blue-50">{labels.write}</a></div>
    </div>
  </section>;
}
