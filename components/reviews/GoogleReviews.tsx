...
export default function GoogleReviews({ className = "" }: { className?: string }) {
  ...
  return <section className={`rounded-[2rem] border border-blue-100 bg-white p-7 shadow-sm sm:p-10 ${className}`}>
    ...
    {loading ? <div className="mt-8 grid gap-4 md:grid-cols-3" aria-live="polite">{[0,1,2].map((item) => <div key={item} className="h-52 animate-pulse rounded-2xl bg-slate-100"><span className="sr-only">{labels.loading}</span></div>)}</div> : null}
