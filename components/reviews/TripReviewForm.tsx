...
export default function TripReviewForm({ initialReference = "", initialEmail = "", locale }: { initialReference?: string; initialEmail?: string; locale?: string }) {
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="block text-sm font-bold text-ink">{t.reference}
        <input required value={reference} onChange={(event) => setReference(event.target.value)} className="mt-1 w-full rounded-xl border border-line px-4 py-3 text-ink" placeholder="DRS-20260801-ABC123" />
      </label>
      <label className="block text-sm font-bold text-ink">{t.email}
        <input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-1 w-full rounded-xl border border-line px-4 py-3 text-ink" placeholder={t.emailPlaceholder} />
      </label>
    </div>
    <div>
      <p className="text-sm font-bold text-ink">{t.rating}</p>
      <div className="mt-1 flex gap-1" role="radiogroup" aria-label={t.ratingGroup}>
        {[1, 2, 3, 4, 5].map((value) => (
          <button key={value} type="button" onClick={() => setRating(value)} aria-pressed={rating >= value} aria-label={`${value} / 5`}>
            <Star size={28} className={value <= rating ? "fill-amber-400 text-amber-400" : "text-line"} />
          </button>
        ))}
      </div>
    </div>
    <label className="block text-sm font-bold text-ink">{t.review}
      <textarea required minLength={1} maxLength={2000} rows={5} value={body} onChange={(event) => setBody(event.target.value)} className="mt-1 w-full rounded-xl border border-line px-4 py-3 text-ink" placeholder={t.reviewPlaceholder} />
    </label>
    {error ? <p role="alert" className="rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-800">{error}</p> : null}
    <button type="submit" disabled={status === "submitting"} className="w-full rounded-2xl bg-ocean-dark px-6 py-4 text-lg font-black text-white hover:brightness-90 disabled:opacity-60">
      {status === "submitting" ? t.submitting : t.submit}
    </button>
    <p className="text-xs leading-5 text-muted">{t.note}</p>
  </form>;
}
