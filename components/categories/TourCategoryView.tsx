...
/** Hurghada category landing page, shared by /hurghada/[category] and the localized /[locale]/hurghada/[category] route. */
export default async function TourCategoryView({ categorySlug, locale = "en" }: { categorySlug: string; locale?: "en" | "de" | "ru" | "ar" | "pl" | "zh" }) {
  ...
  return (
    <main className="min-h-screen bg-surface-muted">
      ...
      <section className="bg-ink px-6 pb-20 pt-32 text-white sm:px-8">
        <div className="mx-auto max-w-6xl">
          <nav aria-label="Breadcrumb" className="text-sm text-muted"><Link href={localePath(locale)} className="hover:text-white">{homeLabel}</Link><span className="px-2" aria-hidden="true">/</span><Link href={destinationPath} className="hover:text-white">{destinationLabel}</Link><span className="px-2" aria-hidden="true">/</span><span className="text-white" aria-current="page">{displayTitle}</span></nav>
          <p className="mt-10 font-semibold uppercase tracking-[0.28em] text-ocean-soft">{ru ? "Экскурсии в Хургаде" : ar ? "رحلات الغردقة" : pl ? "Wycieczki w Hurghadzie" : zh ? "赫尔格达旅游项目" : category.eyebrow}</p>
