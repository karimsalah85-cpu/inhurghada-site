...
export default async function DestinationCategoryPage({ destinationSlug, categorySlug, locale = "en" }: { destinationSlug: string; categorySlug: string; locale?: Locale }) {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="min-h-screen bg-slate-50">
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, "\\u003c") }} />
    <section className="bg-slate-950 px-6 pb-20 pt-32 text-white sm:px-8"><div className="mx-auto max-w-6xl"><nav aria-label="Breadcrumb" className="text-sm text-slate-400"><Link href={localePath(locale)}>{ui.home}</Link><span className="px-2">/</span><Link href={destinationPath}>{destinationName}</Link><span className="px-2">/</span><span className="text-white">{displayTitle}</span></nav><p className="mt-10 font-semibold uppercase tracking-[0.28em] text-cyan-300">{destinationName} · {ui.experiences}</p><h1 className="mt-4 text-4xl font-black sm:text-6xl">{displayTitle} · {destinationName}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">{ui.compare(displayTitle, destinationName)}</p><div className="mt-8 flex flex-wrap gap-3 text-sm font-bold"><span className="rounded-full bg-white/10 px-4 py-2">{ui.clearPrices}</span><span className="rounded-full bg-white/10 px-4 py-2">{ui.localDetails}</span><span className="rounded-full bg-white/10 px-4 py-2">{ui.directSupport}</span></div></div></section>
    <section className="mx-auto max-w-7xl px-6 py-16 sm:px-8"><CategoryTourExplorer tours={tours} locale={locale}/></section>
    {guide ? <section className="border-t border-slate-200 bg-white px-6 py-16 sm:px-8"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-black text-slate-950">{ui.guide}</h2><p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">{guide.intro}</p><div className="mt-8 grid gap-5 md:grid-cols-3">{guide.options.map(([title, text]) => <article key={title} className="rounded-3xl border border-slate-200 bg-slate-50 p-6"><h3 className="text-xl font-black text-slate-950">{title}</h3><p className="mt-3 leading-7 text-slate-600">{text}</p></article>)}</div><div className="mt-10 rounded-3xl border border-cyan-200 bg-cyan-50 p-7"><h2 className="text-2xl font-black text-slate-950">{ui.questions}</h2><div className="mt-4 divide-y divide-cyan-200">{guide.faqs.map(([question, answer]) => <details key={question} className="py-4"><summary className="cursor-pointer font-bold text-slate-950">{question}</summary><p className="mt-3 leading-7 text-slate-700">{answer}</p></details>)}</div></div></div></section> : null}
    <section className="mx-auto max-w-7xl px-6 pb-20 sm:px-8"><Link href={destinationPath} className="inline-flex rounded-full bg-cyan-700 px-6 py-3 font-bold text-white">{ui.allTours(destinationName)} →</Link></section>
  </main>;
}
