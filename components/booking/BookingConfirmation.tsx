...
export default function BookingConfirmation() {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 pb-20 pt-32 sm:px-8"><div className="mx-auto max-w-3xl rounded-[2rem] border border-emerald-200 bg-white p-6 shadow-xl sm:p-10"><CheckCircle2 className="text-emerald-600" size={48}/><p className="mt-5 text-sm font-black uppercase tracking-[0.22em] text-emerald-700">{de ? "Buchung erhalten" : ru ? "Бронирование получено" : ar ? "تم استلام الحجز" : "Booking received"}</p><h1 className="mt-2 text-3xl font-black text-slate-950 sm:text-4xl">{de ? "Vielen Dank" : ru ? "Спасибо" : ar ? "شكراً لك" : "Thank you"}{details?.customerName ? `، ${details.customerName.split(" ")[0]}` : ""}.</h1><p className="mt-4 leading-7 text-slate-600">{de ? "Deine Buchungsanfrage wurde sicher gespeichert. Unser Team bestätigt Verfügbarkeit und Abholung per WhatsApp." : ru ? "Ваша заявка сохранена. Наша команда подтвердит наличие мест и детали трансфера в WhatsApp." : ar ? "تم حفظ طلب الحجز. سيؤكد فريقنا التوفر وتفاصيل الاستلام عبر واتساب." : "Your booking request has been saved securely. Our team will confirm availability and pickup details on WhatsApp."}</p>
