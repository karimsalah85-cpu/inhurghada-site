  ...
  return (
    <form onSubmit={submitTransfer} aria-busy={submitting} className="rounded-3xl bg-white p-6 shadow-2xl md:p-8">
      ...
      <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-5">
        <div className="flex items-start justify-between gap-5"><div><p className="font-bold text-slate-950">{service === "airport" ? tr("Airport one-way fare", "Flughafen – einfache Fahrt", "Тариф в одну сторону из аэропорта") : tr("Senzo Mall one-way fare", "Senzo Mall – einfache Fahrt", "Тариф в одну сторону до Senzo Mall")}</p><p className="mt-1 text-sm leading-6 text-slate-600">{resortSupplement ? resortSupplementText(resortZones.has(pickup) ? pickup : dropoff) : tr("Hurghada base zone—no supplement.", "Hurghada-Basiszone – kein Zuschlag.", "Базовая зона Хургады — без надбавки.")}</p></div><p className="text-3xl font-black text-blue-700">${total.toFixed(2)}</p></div>
        <div className="mt-4 border-t border-blue-200 pt-4 text-sm text-slate-700"><p><strong>{tr("Vehicle:", "Fahrzeug:", "Автомобиль:")}</strong> {vehicle}</p>{service === "airport" ? <p className="mt-1">{tr("1–2 passengers: small car, maximum 2 bags. More than 2 passengers: larger vehicle, maximum 2 bags per person.", "1–2 Fahrgäste: Kleinwagen, maximal 2 Koffer. Mehr als 2 Fahrgäste: größeres Fahrzeug, maximal 2 Koffer pro Person.", "1–2 пассажира: легковой автомобиль, максимум 2 чемодана. Более 2 пассажиров: автомобиль повышенной вместимости, максимум 2 чемодана на человека.")}</p> : <p className="mt-1">{tr("Maximum 4 passengers. Travel bags are not accepted.", "Maximal 4 Fahrgäste. Reisekoffer sind nicht erlaubt.", "Максимум 4 пассажира. Провоз чемоданов не предусмотрен.")}</p>}</div>
      </div>
