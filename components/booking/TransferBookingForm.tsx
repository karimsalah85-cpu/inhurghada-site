...
const arabicTransferCopy: Record<string, string> = {
  ...
  "Transfer request failed. Please try again.": "تعذّر إرسال طلب التوصيل. يرجى المحاولة مرة أخرى.", "Airport one-way fare": "أجرة الاتجاه الواحد من المطار", "Senzo Mall one-way fare": "أجرة الاتجاه الواحد إلى سنزو مول", "Hurghada base zone—no supplement.": "منطقة الغردقة الأساسية - بدون رسوم إضافية.", "Vehicle:": "السيارة:", "1–2 passengers: small car, maximum 2 bags. More than 2 passengers: larger vehicle, maximum 2 bags per person.": "1-2 راكب: سيارة صغيرة، بحد أقصى حقيبتين. أكثر من راكبين: سيارة أكبر، بحد أقصى حقيبتين لكل شخص.", "Maximum 4 passengers. Travel bags are not accepted.": "بحد أقصى 4 ركاب. لا يُسمح بحقائب السفر.",
...
export default function TransferBookingForm({ initialService = "airport" }: { initialService?: TransferService }) {
  ...
  const resortSupplementText = (area: string) => de
    ...
    : ru
    ? `Включает надбавку $7 за ${area}.`
    : ar
    ? `يشمل رسوم إضافية قدرها 7 دولار لمنطقة ${area}.`
    : pl
    ? `Zawiera dopłatę 7 USD za ${area}.`
    : zh
    ? `包含 ${area} 的 7 美元附加费。`
  ...
  async function submitTransfer(event: FormEvent<HTMLFormElement>) {
    ...
    if (service === "airport" && passengerCount <= 2 && bagCount > 2) {
      alert(tr("The small car allows a maximum of 2 travel bags. Reduce the bags or add a third passenger to request a larger vehicle.", "Im Kleinwagen sind maximal 2 Reisekoffer erlaubt. Reduziere die Anzahl oder buche für mindestens 3 Fahrgäste ein größeres Fahrzeug.", "В легковом автомобиле допускается не более 2 чемоданов. Уменьшите число чемоданов или добавьте третьего пассажира, чтобы заказать более вместительный автомобиль.", "تسمح السيارة الصغيرة بحقيبتي سفر كحد أقصى. قلل عدد الحقائب أو أضف راكباً ثالثاً لطلب سيارة أكبر."));
      return;
    }
    if (service === "airport" && passengerCount > 2 && bagCount > passengerCount * 2) {
      alert(tr("The larger vehicle allows a maximum of 2 travel bags per passenger.", "Im größeren Fahrzeug sind maximal 2 Reisekoffer pro Fahrgast erlaubt.", "В автомобиле повышенной вместимости допускается не более 2 чемоданов на пассажира.", "تسمح السيارة الأكبر بحقيبتي سفر كحد أقصى لكل راكب."));
      return;
    }
    const phoneCheck = validatePhoneNumber(phone, "EG");
    if (!phoneCheck.valid) {
      alert(tr("Please enter a valid WhatsApp phone number including the country code.", "Bitte gib eine gültige WhatsApp-Telefonnummer inklusive Landesvorwahl ein.", "Пожалуйста, введите действительный номер WhatsApp с кодом страны."));
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        alert(data.error || tr("Transfer request failed. Please try again.", "Transferanfrage fehlgeschlagen. Bitte versuche es erneut.", "Не удалось отправить заявку на трансфер. Попробуйте ещё раз."));
        return;
      }
      ...
      window.sessionStorage.setItem(confirmationStorageKey(data.reference), JSON.stringify({
        ...
        date,
        time,
        travelers: `${passengers} ${de ? "Fahrgäste" : ru ? "пасс." : ar ? "ركاب" : pl ? "pasażerów" : zh ? "位乘客" : "passengers"}`,
  ...
  return (
    <form onSubmit={submitTransfer} aria-busy={submitting} className="rounded-3xl bg-white p-6 shadow-2xl md:p-8">
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        <p className="text-xs text-muted sm:col-span-2"><RequiredMark/> {tr("Required field", "Pflichtfeld", "Обязательное поле")}</p>
...
function Field({ icon, label, children, required = false }: { icon: ReactNode; label: string; children: ReactNode; required?: boolean }) {
  return <label className="block text-sm font-medium text-ink"><span className="mb-2 flex items-center gap-2 text-ink"><span className="text-ocean">{icon}</span>{label}{required ? <RequiredMark/> : null}</span><div className="[&>input]:w-full [&>input]:rounded-xl [&>input]:border [&>input]:border-line [&>input]:p-3 [&>input]:outline-none [&>input]:focus:border-ocean [&>select]:w-full [&>select]:rounded-xl [&>select]:border [&>select]:border-line [&>select]:bg-white [&>select]:p-3 [&>select]:outline-none [&>select]:focus:border-ocean">{children}</div></label>;
}
