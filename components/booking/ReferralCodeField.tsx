...
export function useReferralCode() {
  ...
  return { code, setCode, valid: !code || code.length < 10 || isValidReferralCode(code.toUpperCase()) };
}
...
export default function ReferralCodeField({ referral, locale, disabled = false }: { referral: ReturnType<typeof useReferralCode>; locale: Locale; disabled?: boolean }) {
  ...
  return <div className="rounded-2xl border border-line p-4">
    <label className="block text-sm font-bold text-ink">{copy.checkoutCodeLabel}
      <input dir="ltr" value={referral.code} maxLength={16} disabled={disabled} onChange={(event) => referral.setCode(event.target.value.toUpperCase())} placeholder="DRS-XXXXXX" autoComplete="off" className="mt-2 w-full rounded-xl border border-line px-3 py-3 uppercase"/>
    </label>
