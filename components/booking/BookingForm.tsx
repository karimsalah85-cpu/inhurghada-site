  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <button disabled={unavailable || boatOverCapacity || unavailableWeekday} type="button" onClick={beginCheckout} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-brand-orange-cta py-4 font-bold text-white hover:brightness-90 disabled:cursor-not-allowed disabled:bg-slate-400">{unavailable ? tr("Sold out", "Ausverkauft", "Мест нет") : tr("Book now", "Jetzt buchen", "Забронировать")} <Users size={18}/></button>
        ...
        <label className="block text-sm font-bold text-slate-800">{tr("WhatsApp number", "WhatsApp-Nummer", "Номер WhatsApp")} <RequiredMark/><input required type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={destinationSlug === "jeddah" ? "+966 5X XXX XXXX" : "+20 103 080 9150"} /></label>
        {meetingPointProduct ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><p className="font-black">{ar ? "نقطة التجمع" : "Meeting point"}</p><p className="mt-1">{meetingPoint || (ar ? "سيتم تأكيد نقطة التجمع قبل الرحلة." : "The meeting point will be confirmed before the trip.")}</p><p className="mt-2 text-xs">{ar ? "لا يلزم إدخال عنوان فندق لهذه الرحلة." : "No hotel or pickup address is required for this trip."}</p></div> : <label className="block text-sm font-bold text-slate-800">{tr("Pickup location", "Abholort", "Место трансфера")} {pickupOptional ? <span className="font-normal text-slate-500">({tr("optional", "optional", "необязательно")})</span> : <RequiredMark/>}<div className="relative mt-1"><Hotel className="absolute left-3 top-3 text-slate-400" size={18}/><input required={!pickupOptional} value={hotel} onChange={(event) => setHotel(event.target.value)} className="w-full rounded-xl border border-slate-300 bg-white py-3 pl-10 pr-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={tr("Hotel name or full pickup address", "Hotelname oder vollständige Abholadresse", "Название отеля или полный адрес")} /></div>{pickupOptional ? <span className="mt-1 block text-xs font-normal text-slate-500">{ar ? "اختياري للرحلات في السعودية؛ سنؤكد تفاصيل نقطة التجمع عبر واتساب." : "Optional for Saudi trips; meeting details are confirmed by WhatsApp."}</span> : pickupZones.length ? <span className="mt-1 block text-xs font-normal text-emerald-700">Included pickup zones: {pickupZones.join(", ")}.</span> : <span className="mt-1 block text-xs font-normal text-slate-500">{tr("Required so we can confirm your pickup on WhatsApp.", "Erforderlich, damit wir deine Abholung per WhatsApp bestätigen können.", "Нужно для подтверждения трансфера в WhatsApp.")}</span>}</label>}
