...
import { groupRate, groupTierRanges, type GroupPriceTier } from "@/lib/group-pricing";
...
import { isOperatingDate, isQuadTour, nextDepartures, nextOperatingDate, tourMinimumAge, type FulfillmentType } from "@/lib/tour-booking";
...
const STEP_LABELS: Record<Language, [string, string, string]> = {
  en: ["Trip", "Your details", "Confirm"],
  ar: ["الرحلة", "بياناتك", "التأكيد"],
  de: ["Ausflug", "Deine Angaben", "Bestätigen"],
  ru: ["Поездка", "Ваши данные", "Подтверждение"],
  pl: ["Wycieczka", "Twoje dane", "Potwierdzenie"],
  zh: ["行程", "您的信息", "确认"],
};

function BookingSteps({ current, language }: { current: 1 | 2 | 3; language: Language }) {
  const labels = STEP_LABELS[language] ?? STEP_LABELS.en;
  return (
    <ol className="mb-6 flex items-center gap-2 text-xs font-semibold" aria-label={`Step ${current} of 3`}>
      {labels.map((label, index) => {
        const step = index + 1;
        const state = step < current ? "done" : step === current ? "current" : "upcoming";
        return (
          <li key={label} className="flex flex-1 items-center gap-2 last:flex-none">
            <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] tabular-nums ${state === "current" ? "bg-ocean-dark text-white" : state === "done" ? "bg-emerald-500 text-white" : "bg-surface-muted text-muted"}`}>
              {state === "done" ? <Check size={13} aria-hidden="true" /> : step}
            </span>
            <span className={state === "upcoming" ? "truncate text-muted" : "truncate text-ink"}>{label}</span>
            {step < 3 ? <span className="hidden h-px flex-1 bg-line sm:block" aria-hidden="true" /> : null}
          </li>
        );
      })}
    </ol>
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3">
      ...
      <div className="flex items-center overflow-hidden rounded-xl border border-line bg-white shadow-sm">
        <button type="button" aria-label={`Remove ${label}`} onClick={() => onChange(Math.max(0, value - 1))} className="p-2.5 text-ink hover:bg-surface-muted"><CircleMinus size={18} /></button>
        <span className="w-9 text-center font-bold text-ink">{value}</span>
        <button type="button" aria-label={`Add ${label}`} disabled={value >= 30} onClick={() => onChange(Math.min(30, value + 1))} className="p-2.5 text-ocean-dark hover:bg-ocean-tint disabled:opacity-40"><CirclePlus size={18} /></button>
      </div>
    </div>
  );
}
...
const arabicBookingCopy: Record<string, string> = {
  ...
  "We'll use this number for booking confirmation and pickup details.": "سنستخدم هذا الرقم لتأكيد الحجز وتفاصيل الاستلام.", "Please enter a valid WhatsApp phone number including the country code.": "يرجى إدخال رقم واتساب صحيح مع رمز الدولة.",
};
...
export default function BookingForm({ tourName, tourSlug, destinationSlug = "hurghada", pickupZones = [], price, originalPrice, priceUnit, pricingMode = "per-person", duration, location, participantPricing, groupPricing, availableTimes, ageBands, boatOptions, packageOptions, entrancePricing, bookingExtras = [], requiresMarinaTransferChoice = false, bookingLeadTime, currency = "USD", operatingWeekdays, fulfillmentType = "hotel_pickup", meetingPoint }: BookingFormProps) {
  ...
  const tr = (en: string, deText: string, ruText: string, arText = arabicBookingCopy[en] || en) => {
    if (en === "Travelers and island entrance" && !entrancePricing) return ar ? "المسافرون" : de ? "Reisende" : ru ? "Путешественники" : pl ? "Uczestnicy" : zh ? "出行人数" : "Travelers";
    return de ? deText : ru ? ruText : ar ? arText : pl ? polishBookingCopy[en] || en : zh ? chineseBookingCopy[en] || en : en;
  };
  ...
  const adultPrice = selectedPackage ? selectedPackage.price : groupRate(groupPricing, adults + youth) ?? baseAdultPrice;
  const requiresDivingLicense = tourSlug === "full-day-diving";
  const minimumAge = tourMinimumAge(tourSlug);
  const requiresQuadMinimumAge = minimumAge !== undefined;
  const quadTour = isQuadTour(tourSlug);
  const riderAge = {
    title: de ? `Mindestalter: ${minimumAge} Jahre` : ru ? `Минимальный возраст: ${minimumAge} лет` : ar ? `الحد الأدنى للعمر: ${minimumAge} سنوات` : pl ? `Minimalny wiek: ${minimumAge} lat` : zh ? `最低年龄：${minimumAge}岁` : `Minimum age: ${minimumAge} years`,
    detail: de ? `Alle Reiter müssen am Tag des Ausritts mindestens ${minimumAge} Jahre alt sein. Kinder unter ${minimumAge} Jahren können nicht teilnehmen.` : ru ? `Всем всадникам в день прогулки должно быть не менее ${minimumAge} лет. Дети младше ${minimumAge} лет не допускаются.` : ar ? `يجب ألا يقل عمر كل راكب عن ${minimumAge} سنوات يوم الركوب. لا يمكن للأطفال دون ${minimumAge} سنوات المشاركة.` : pl ? `Każdy jeździec musi mieć w dniu jazdy co najmniej ${minimumAge} lat. Dzieci poniżej ${minimumAge} lat nie mogą uczestniczyć.` : zh ? `骑行当天每位骑手必须年满${minimumAge}岁。${minimumAge}岁以下儿童不能参加。` : `Every rider must be at least ${minimumAge} years old on the day of the ride. Children under ${minimumAge} cannot join.`,
    confirm: de ? `Ich bestätige, dass alle Reiter mindestens ${minimumAge} Jahre alt sind.` : ru ? `Я подтверждаю, что всем всадникам не менее ${minimumAge} лет.` : ar ? `أؤكد أن عمر كل راكب لا يقل عن ${minimumAge} سنوات.` : pl ? `Potwierdzam, że każdy jeździec ma co najmniej ${minimumAge} lat.` : zh ? `我确认每位骑手均已年满${minimumAge}岁。` : `I confirm that every rider is at least ${minimumAge} years old.`,
    error: de ? `Alle Reiter müssen mindestens ${minimumAge} Jahre alt sein.` : ru ? `Всем всадникам должно быть не менее ${minimumAge} лет.` : ar ? `يجب ألا يقل عمر كل راكب عن ${minimumAge} سنوات.` : pl ? `Każdy jeździec musi mieć co najmniej ${minimumAge} lat.` : zh ? `每位骑手必须年满${minimumAge}岁。` : `Every rider must be at least ${minimumAge} years old.`,
  };
  ...
  const pickupOptional = destinationSlug === "jeddah";
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (requiresQuadMinimumAge && !quadMinimumAgeConfirmed) {
      setError((quadTour ? tr("Every quad-tour participant must be at least 9 years old.", "Alle Teilnehmer der Quad-Tour müssen mindestens 9 Jahre alt sein.", "Каждому участнику тура на квадроциклах должно быть не менее 9 лет.") : riderAge.error));
      return;
    }
    const phoneCheck = validatePhoneNumber(phone, destinationSlug === "jeddah" ? "SA" : "EG");
    if (!phoneCheck.valid) {
      setError(tr("Please enter a valid WhatsApp phone number including the country code.", "Bitte gib eine gültige WhatsApp-Telefonnummer inklusive Landesvorwahl ein.", "Пожалуйста, введите действительный номер WhatsApp с кодом страны."));
      return;
    }
  ...
  return (
    <div className="rounded-3xl border border-line bg-white p-5 shadow-xl sm:p-7">
      <BookingSteps current={step === "select" ? 1 : 2} language={language} />
      <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-muted">{tr("From", "Ab", "От")} {originalPrice && Number(originalPrice) > adultPrice ? <span className="mr-2 text-sm text-muted line-through">{formatPrice(originalPrice, currency)}</span> : null}<span className="text-2xl font-black text-ink">{formatPrice(String(adultPrice), currency)}</span> / {priceUnit || tr("person", "Person", "человека")}</p><p className="mt-1 text-sm font-medium text-emerald-700">{meetingPointProduct ? (ar ? "سعر واضح · نقطة التجمع موضحة قبل الحجز" : "Clear local price · meeting point shown before booking") : tr("Clear local price · pickup confirmed after booking", "Klarer lokaler Preis · Abholung nach Buchung bestätigt", "Понятная местная цена · трансфер подтверждается после бронирования")}</p></div><ShieldCheck className="text-emerald-600" /></div>
      {step === "select" ? <>
        <div className="mt-6 space-y-4">
          <div className="block text-sm font-bold text-ink">{tr("Date", "Datum", "Дата")} <RequiredMark/><BookableDatePicker value={date} minimumDate={minimumDate} operatingWeekdays={operatingWeekdays} locale={language} onChange={(selectedDate) => { setDate(selectedDate); trackEvent("date_selected", { item_id: tourSlug, date: selectedDate }); }}/></div>
          {operatingWeekdays?.length ? <div className="flex flex-wrap gap-2" aria-label={ar ? "المواعيد القادمة" : "Next departures"}>{upcomingDepartures.map((departure) => <button key={departure} type="button" onClick={() => setDate(departure)} className={`rounded-full border px-3 py-2 text-xs font-bold ${date === departure ? "border-ocean-dark bg-ocean-dark text-white" : "border-line bg-white text-ink"}`}>{new Intl.DateTimeFormat(ar ? "ar-SA" : "en-SA", { weekday: "short", month: "short", day: "numeric", timeZone: "Asia/Riyadh" }).format(new Date(`${departure}T12:00:00Z`))}</button>)}</div> : null}
          ...
          <label className="block text-sm font-bold text-ink">{tr("Time", "Uhrzeit", "Время")} <RequiredMark/><div className="relative mt-1"><Clock3 className="absolute left-3 top-3 text-muted" size={18}/><select required value={time} onChange={(event) => setTime(event.target.value)} className="w-full appearance-none rounded-xl border border-line bg-white px-10 py-3 font-medium text-ink outline-none transition focus:border-ocean focus:ring-4 focus:ring-ocean-tint">{times.map((option) => <option key={option}>{option === "Time confirmed by WhatsApp" ? tr(option, "Uhrzeit wird per WhatsApp bestätigt", "Время подтверждается в WhatsApp") : option}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-muted" size={18}/></div></label>
        </div>
        {requiresDivingLicense ? <div className="mt-5 rounded-2xl border border-ocean-soft bg-ocean-tint p-4 text-sm leading-6 text-ocean-dark"><p className="font-black">{tr("Valid diving license required", "Gültiger Tauchschein erforderlich", "Требуется действующий сертификат дайвера")}</p><p className="mt-1">{tr("Every diver must hold a valid scuba diving license and bring proof on the trip.", "Jeder Taucher muss einen gültigen Tauchschein besitzen und den Nachweis mitbringen.", "У каждого дайвера должен быть действующий сертификат для подводного плавания; документ нужно взять с собой в поездку.")}</p></div> : null}
        {requiresQuadMinimumAge ? <div className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><p className="font-black">{(quadTour ? tr("Minimum age: 9 years", "Mindestalter: 9 Jahre", "Минимальный возраст: 9 лет") : riderAge.title)}</p><p className="mt-1">{(quadTour ? tr("Every participant must be at least 9 years old on the day of the tour.", "Alle Teilnehmer müssen am Tag der Tour mindestens 9 Jahre alt sein.", "Каждому участнику должно быть не менее 9 лет в день тура.") : riderAge.detail)}</p></div> : null}
        {boatOptions?.length ? <fieldset className="mt-6 rounded-2xl border border-ocean-soft bg-ocean-tint/60 p-4"><legend className="px-1 text-sm font-black text-ink">{tr("Select your private boat", "Wähle dein privates Boot", "Выберите частную лодку")}</legend><div className="space-y-2">{boatOptions.map((option) => <label key={option.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl bg-white p-3 text-sm"><span className="flex items-center gap-3"><input type="radio" name="boat-option" checked={selectedBoatOption === option.id} onChange={() => setSelectedBoatOption(option.id)} className="accent-ocean-dark"/><span>{option.label}{option.extraHourPrice ? <small className="mt-1 block text-xs text-muted">{tr("Extra hour", "Zusätzliche Stunde", "Дополнительный час")}: {formatPrice(option.extraHourPrice)}</small> : null}</span></span><strong>{formatPrice(option.price)}</strong></label>)}</div></fieldset> : null}
        {packageOptions && packageOptions.length > 1 ? <fieldset className="mt-6 rounded-2xl border border-ocean-soft bg-ocean-tint/60 p-4"><legend className="px-1 text-sm font-black text-ink">{tr("Select your package", "Paket auswählen", "Выберите пакет")}</legend><div className="space-y-2">{packageOptions.map((option) => <label key={option.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl bg-white p-3 text-sm"><span className="flex items-center gap-3"><input type="radio" name="package-option" checked={selectedPackageOption === option.id} onChange={() => setSelectedPackageOption(option.id)} className="accent-ocean-dark"/><span>{option.label}</span></span><strong>{formatPrice(String(option.price), currency)}</strong></label>)}</div></fieldset> : null}
        <div className="mt-6 rounded-2xl border border-line bg-surface-muted/60 px-4"><p className="pt-4 text-sm font-bold text-ink">{tr("Travelers and island entrance", "Reisende und Insel-Eintritt", "Путешественники и вход на остров")}</p><Counter label={ageBands?.adults || tr("Adults", "Erwachsene", "Взрослые")} description={entrancePricing ? `${formatPrice(entrancePricing.adults)} ${tr("entrance each", "Eintritt pro Person", "билет за каждого")}` : pricingMode === "per-booking" ? `${formatPrice(String(adultPrice), currency)} ${tr("total per booking", "gesamt pro Buchung", "за всё бронирование")}` : `${formatPrice(String(adultPrice), currency)} ${tr("each", "pro Person", "за человека")}`} value={adults} onChange={setAdults}/>{groupPricing?.length ? <div className="pb-3 text-xs text-muted"><p className="font-bold text-ink">{tr("Group price per person", "Gruppenpreis pro Person", "Групповая цена за человека")}</p><ul className="mt-1 space-y-0.5">{groupTierRanges(groupPricing).map((tier) => { const current = adults + youth >= tier.from && (tier.to === undefined || adults + youth <= tier.to); return <li key={tier.minTravelers} className={`flex justify-between ${current ? "font-bold text-ocean-dark" : ""}`}><span>{tier.label} {tr("travelers", "Reisende", "чел.")}</span><span>{formatPrice(String(tier.pricePerPerson), currency)}</span></li>; })}</ul></div> : null}{(entrancePricing || youthPrice !== undefined) && <Counter label={ageBands?.children || tr("Youth (4–10)", "Kinder (4–10)", "Дети (4–10)")} description={`${entrancePricing ? formatPrice(entrancePricing.youth) : formatPrice(String(youthPrice ?? 0), currency)} ${tr("each", "pro Kind", "за ребёнка")}`} value={youth} onChange={setYouth}/>} {infantPrice !== undefined && <Counter label={ageBands?.infants || tr("Infants", "Kleinkinder", "Младенцы")} description={tr("Free of charge", "Kostenlos", "Бесплатно")} value={infants} onChange={setInfants}/>} {boatOverCapacity ? <p className="pb-4 text-sm font-bold text-rose-700">{boatCapacityNotice(selectedBoat?.capacity)}</p> : null}</div>
        ...
        <div className="mt-5 flex items-end justify-between gap-3 border-t pt-5"><div className="min-w-0"><p className="font-bold text-ink">{tr("Total", "Gesamtpreis", "Итого")}</p><p className="text-xs text-muted">{tr("Cash on arrival · no online payment", "Barzahlung bei Ankunft · keine Online-Zahlung", "Оплата наличными по прибытии · без онлайн-оплаты")}</p></div><p className="shrink-0 whitespace-nowrap text-2xl font-black text-ink sm:text-3xl">{formatPrice(String(promo.total), currency)}</p></div>
        ...
        }} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-ocean-soft bg-ocean-tint py-4 font-bold text-ocean-dark hover:bg-ocean-soft">{tr("Add to trip cart", "Zum Reisewarenkorb", "Добавить в корзину", "أضف إلى سلة الرحلات")} <ShoppingCart size={18}/></button>
        ...
        <label className="block text-sm font-bold text-ink">{tr("Email address", "E-Mail-Adresse", "Электронная почта")} <RequiredMark/><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" className="mt-1 w-full rounded-xl border border-line bg-white p-3 font-normal text-ink outline-none placeholder:text-muted focus:border-ocean focus:ring-4 focus:ring-ocean-tint" placeholder="you@example.com" /></label>
        <label className="block text-sm font-bold text-ink">{tr("WhatsApp number", "WhatsApp-Nummer", "Номер WhatsApp")} <RequiredMark/><PhoneNumberInput
          defaultCountry={destinationSlug === "jeddah" ? "SA" : "EG"}
          value={phone}
          onChange={(next) => setPhone(next)}
          language={language}
          hasError={Boolean(error)}
          required
          ariaLabel={tr("WhatsApp number", "WhatsApp-Nummer", "Номер WhatsApp")}
          placeholder={destinationSlug === "jeddah" ? "5X XXX XXXX" : "10 1234 5678"}
        /><span className="mt-1 block text-xs font-normal text-muted">{tr("We'll use this number for booking confirmation and pickup details.", "Wir verwenden diese Nummer für die Buchungsbestätigung und Abholdetails.", "Мы используем этот номер для подтверждения бронирования и деталей трансфера.")}</span></label>
        {meetingPointProduct ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><p className="font-black">{ar ? "نقطة التجمع" : "Meeting point"}</p><p className="mt-1">{meetingPoint || (ar ? "سيتم تأكيد نقطة التجمع قبل الرحلة." : "The meeting point will be confirmed before the trip.")}</p><p className="mt-2 text-xs">{ar ? "لا يلزم إدخال عنوان فندق لهذه الرحلة." : "No hotel or pickup address is required for this trip."}</p></div> : <label className="block text-sm font-bold text-ink">{tr("Pickup location", "Abholort", "Место трансфера")} {pickupOptional ? <span className="font-normal text-muted">({tr("optional", "optional", "необязательно")})</span> : <RequiredMark/>}<div className="relative mt-1"><Hotel className="absolute left-3 top-3 text-muted" size={18}/><input required={!pickupOptional} value={hotel} onChange={(event) => setHotel(event.target.value)} className="w-full rounded-xl border border-line bg-white py-3 pl-10 pr-3 font-normal text-ink outline-none placeholder:text-muted focus:border-ocean focus:ring-4 focus:ring-ocean-tint" placeholder={tr("Hotel name or full pickup address", "Hotelname oder vollständige Abholadresse", "Название отеля или полный адрес")} /></div>{pickupOptional ? <span className="mt-1 block text-xs font-normal text-muted">{ar ? "اختياري للرحلات في السعودية؛ سنؤكد تفاصيل نقطة التجمع عبر واتساب." : "Optional for Saudi trips; meeting details are confirmed by WhatsApp."}</span> : pickupZones.length ? <span className="mt-1 block text-xs font-normal text-emerald-700">Included pickup zones: {pickupZones.join(", ")}.</span> : <span className="mt-1 block text-xs font-normal text-muted">{tr("Required so we can confirm your pickup on WhatsApp.", "Erforderlich, damit wir deine Abholung per WhatsApp bestätigen können.", "Нужно для подтверждения трансфера в WhatsApp.")}</span>}</label>}
        ...
        {requiresDivingLicense ? <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-ocean-soft bg-ocean-tint p-4 text-sm leading-6 text-ocean-dark"><input type="checkbox" required checked={divingLicenseConfirmed} onChange={(event) => setDivingLicenseConfirmed(event.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-ocean-dark" /><span><strong>{tr("Valid diving license required", "Gültiger Tauchschein erforderlich", "Требуется действующий сертификат дайвера")} <RequiredMark/></strong><span className="mt-1 block">{tr("I confirm that every diver holds a valid scuba diving license and will bring proof on the trip.", "Ich bestätige, dass jeder Taucher einen gültigen Tauchschein besitzt und den Nachweis zum Ausflug mitbringt.", "Я подтверждаю, что у каждого дайвера есть действующий сертификат для подводного плавания, который он возьмёт с собой в поездку.")}</span></span></label> : null}
        {requiresQuadMinimumAge ? <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><input type="checkbox" required checked={quadMinimumAgeConfirmed} onChange={(event) => setQuadMinimumAgeConfirmed(event.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-amber-700" /><span><strong>{(quadTour ? tr("Minimum age: 9 years", "Mindestalter: 9 Jahre", "Минимальный возраст: 9 лет") : riderAge.title)} <RequiredMark/></strong><span className="mt-1 block">{(quadTour ? tr("I confirm that every participant is at least 9 years old.", "Ich bestätige, dass alle Teilnehmer mindestens 9 Jahre alt sind.", "Я подтверждаю, что всем участникам не менее 9 лет.") : riderAge.confirm)}</span></span></label> : null}
        <label className="block text-sm font-bold text-ink">{tr("Preferred guide language", "Bevorzugte Sprache des Reiseführers", "Предпочитаемый язык гида")}<select value={guideLanguage} onChange={(event) => setGuideLanguage(event.target.value)} className="mt-1 w-full rounded-xl border border-line bg-white p-3 font-normal text-ink outline-none focus:border-ocean focus:ring-4 focus:ring-ocean-tint"><option>English</option><option>Arabic</option><option>German</option><option>Russian</option><option>Polish</option><option>Chinese</option></select></label>
