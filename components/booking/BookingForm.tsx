...
export default function BookingForm({ tourName, tourSlug, destinationSlug = "hurghada", pickupZones = [], price, originalPrice, priceUnit, pricingMode = "per-person", duration, location, participantPricing, availableTimes, ageBands, boatOptions, entrancePricing, bookingExtras = [], requiresMarinaTransferChoice = false, bookingLeadTime, currency = "USD", operatingWeekdays }: BookingFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (boatOverCapacity) { setError(tr(`The selected boat accepts up to ${selectedBoat?.capacity} passengers.`, `Das gewählte Boot ist für höchstens ${selectedBoat?.capacity} Personen geeignet.`, `Выбранная лодка рассчитана максимум на ${selectedBoat?.capacity} пассажиров.`, ar ? `القارب المختار يتسع حتى ${selectedBoat?.capacity} ركاب.` : undefined)); return; }
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4"><p className="font-bold text-slate-900">{tourName}</p><p className="mt-1 text-sm text-slate-600">{date} {de ? "um" : "at"} {time} · {travelerText}</p><p className="mt-2 font-black text-blue-700">{formatPrice(String(total), currency)} · {de ? "Barzahlung bei Ankunft" : "Cash on arrival"}</p></div>
