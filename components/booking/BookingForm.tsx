...
export default function BookingForm({ tourName, tourSlug, price, duration, location, participantPricing, availableTimes }: BookingFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (requiresQuadMinimumAge && !quadMinimumAgeConfirmed) {
      setError(de ? "Alle Teilnehmer der Quad-Tour müssen mindestens 9 Jahre alt sein." : "Every quad-tour participant must be at least 9 years old.");
      return;
    }
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        <div className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-bold text-slate-700">{de ? "Uhrzeit" : "Time"} <RequiredMark/><div className="relative mt-1"><Clock3 className="absolute left-3 top-3 text-slate-400" size={18}/><select required value={time} onChange={(event) => setTime(event.target.value)} className="w-full appearance-none rounded-xl border border-slate-300 bg-white px-10 py-3 font-medium text-slate-950 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">{times.map((option) => <option key={option}>{option}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-slate-400" size={18}/></div></label>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50/60 px-4"><p className="pt-4 text-sm font-bold text-slate-900">{de ? "Paket auswählen" : "Select your package"}</p><Counter label={de ? "Erwachsene" : "Adults"} description={`${formatPrice(String(adultPrice))} ${de ? "pro Person" : "each"}`} value={adults} onChange={setAdults}/>{youthPrice !== undefined && <Counter label={requiresQuadMinimumAge ? (de ? "Jugendliche (9–10)" : "Youth (9–10)") : (de ? "Kinder (4–10)" : "Youth (4–10)")} description={`${formatPrice(String(youthPrice))} ${de ? "pro Kind" : "each"}`} value={youth} onChange={setYouth}/>} {infantPrice !== undefined && <Counter label={de ? "Kleinkinder" : "Infants"} description={de ? "Kostenlos" : "Free of charge"} value={infants} onChange={setInfants}/>}</div>
        ...
        <label className="block text-sm font-bold text-slate-800">{de ? "Vollständiger Name" : "Full name"} <RequiredMark/><input required value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={de ? "Vollständigen Namen eingeben" : "Enter your full name"} /></label>
        <label className="block text-sm font-bold text-slate-800">{de ? "E-Mail-Adresse" : "Email address"} <RequiredMark/><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder="you@example.com" /></label>
        <label className="block text-sm font-bold text-slate-800">{de ? "WhatsApp-Nummer" : "WhatsApp number"} <RequiredMark/><input required type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder="+20 103 080 9150" /></label>
        <label className="block text-sm font-bold text-slate-800">{de ? "Abholort" : "Pickup location"} <RequiredMark/><div className="relative mt-1"><Hotel className="absolute left-3 top-3 text-slate-400" size={18}/><input required value={hotel} onChange={(event) => setHotel(event.target.value)} className="w-full rounded-xl border border-slate-300 bg-white py-3 pl-10 pr-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={de ? "Hotelname oder vollständige Abholadresse" : "Hotel name or full pickup address"} /></div><span className="mt-1 block text-xs font-normal text-slate-500">{de ? "Erforderlich, damit wir deine Abholung per WhatsApp bestätigen können." : "Required so we can confirm your pickup on WhatsApp."}</span></label>
        ...
        <label className="block text-sm font-bold text-slate-800">{de ? "Besondere Wünsche" : "Special requests"} <span className="font-normal text-slate-500">({de ? "optional" : "optional"})</span><textarea value={message} onChange={(event) => setMessage(event.target.value)} className="mt-1 h-20 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal text-slate-950 outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={de ? "Gibt es etwas, das wir wissen sollten?" : "Anything we should know?"} /></label>
