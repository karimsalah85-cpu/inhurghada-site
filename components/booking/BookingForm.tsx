  ...
  return (
    <div className="rounded-3xl border border-line bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <label className="block text-sm font-bold text-ink">{tr("WhatsApp number", "WhatsApp-Nummer", "Номер WhatsApp")} <RequiredMark/><input required type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" className="mt-1 w-full rounded-xl border border-line bg-white p-3 font-normal text-ink outline-none placeholder:text-muted focus:border-ocean focus:ring-4 focus:ring-ocean-tint" placeholder={destinationSlug === "jeddah" ? "+966 5X XXX XXXX" : "+20 103 080 9150"} /><span className="mt-1 block text-xs font-normal text-muted">{tr("We'll use this number for booking confirmation and pickup details.", "Wir verwenden diese Nummer für die Buchungsbestätigung und Abholdetails.", "Мы используем этот номер для подтверждения бронирования и деталей трансфера.")}</span></label>
