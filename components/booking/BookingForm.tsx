...
const upsells: Record<string, { id: string; price: number; en: string; de: string; ru: string; zh: string }[]> = {
  "orange-bay": [{ id: "remote-pickup", price: 6.84, en: "Pickup from Makadi Bay, Soma Bay, El Gouna, Sahl Hasheesh or Safaga (€6 per booking)", de: "Abholung von Makadi Bay, Soma Bay, El Gouna, Sahl Hasheesh oder Safaga (6 € pro Buchung)", ru: "Трансфер из Макади-Бей, Сома-Бей, Эль-Гуны, Сахл-Хашиша или Сафаги (6 € за бронирование)", zh: "马卡迪湾、索马湾、艾尔古纳、萨尔哈希什或萨法加接送（每笔预订 6 欧元）" }],
...
export default function BookingForm({ tourName, tourSlug, price, duration, location, participantPricing, availableTimes, ageBands }: BookingFormProps) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50/60 px-4"><p className="pt-4 text-sm font-bold text-slate-900">{tr("Select your package", "Paket auswählen", "Выберите пакет")}</p><Counter label={ageBands?.adults || tr("Adults", "Erwachsene", "Взрослые")} description={`${formatPrice(String(adultPrice))} ${tr("each", "pro Person", "за человека")}`} value={adults} onChange={setAdults}/>{youthPrice !== undefined && <Counter label={ageBands?.children || (requiresQuadMinimumAge ? tr("Youth (9–10)", "Jugendliche (9–10)", "Дети (9–10)") : tr("Youth (4–10)", "Kinder (4–10)", "Дети (4–10)"))} description={`${formatPrice(String(youthPrice))} ${tr("each", "pro Kind", "за ребёнка")}`} value={youth} onChange={setYouth}/>} {infantPrice !== undefined && <Counter label={ageBands?.infants || tr("Infants", "Kleinkinder", "Младенцы")} description={tr("Free of charge", "Kostenlos", "Бесплатно")} value={infants} onChange={setInfants}/>}</div>
