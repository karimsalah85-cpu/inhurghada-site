  ...
  return (
    <div className="rounded-3xl border border-line bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <label className="block text-sm font-bold text-ink">{tr("WhatsApp number", "WhatsApp-Nummer", "Номер WhatsApp")} <RequiredMark/><input required type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" className="mt-1 w-full rounded-xl border border-line bg-white p-3 font-normal text-ink outline-none placeholder:text-muted focus:border-ocean focus:ring-4 focus:ring-ocean-tint" placeholder={destinationSlug === "jeddah" ? "+966 5X XXX XXXX" : "+20 103 080 9150"} /></label>
      ...
      {step === "select" ? <>
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-white/95 px-4 py-3 shadow-[0_-12px_30px_-18px_rgba(15,23,42,0.7)] backdrop-blur lg:hidden">
          <div className="mx-auto flex max-w-3xl items-center gap-3">
            <div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">{tr("Total", "Gesamtpreis", "Итого")}</p><p className="truncate text-xl font-black text-ink">{formatPrice(String(total), currency)}</p></div>
            {tourSlug === "jeddah-yacht-sunset-cruise" ? <ShareTripButton locale={language} tourSlug={tourSlug} date={date} compact/> : null}
            <button disabled={unavailable || boatOverCapacity || unavailableWeekday} type="button" onClick={beginCheckout} className="flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-brand-orange-cta px-5 py-3 font-bold text-white shadow-lg shadow-brand-orange-cta/20 hover:brightness-90 disabled:cursor-not-allowed disabled:bg-muted">{unavailable ? tr("Sold out", "Ausverkauft", "Мест нет") : tr("Book now", "Jetzt buchen", "Забронировать")} <Users size={18}/></button>
          </div>
        </div>
      </> : null}
    </div>
