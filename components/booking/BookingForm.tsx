  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      {step === "select" ? <>
        ...
        <p className="text-center text-xs leading-5 text-slate-500">{meetingPointProduct ? (ar ? "تم إرسال طلبك؛ سيؤكد فريقنا التوفر النهائي عبر واتساب." : "Your request is submitted; our team confirms final availability on WhatsApp.") : tr("Your booking request is submitted now. You pay cash when you arrive; we confirm pickup via WhatsApp.", "Deine Buchungsanfrage wird jetzt gesendet. Du bezahlst bei Ankunft bar; wir bestätigen die Abholung per WhatsApp.", "Заявка отправляется сейчас. Оплата наличными по прибытии; трансфер мы подтвердим в WhatsApp.")}</p>
      </form>}
      {step === "select" ? <>
