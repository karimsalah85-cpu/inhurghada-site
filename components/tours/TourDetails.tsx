    ...
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-line bg-white p-8 shadow-sm">
        ...
        {hasLiveReviews ? <div className="mt-8 grid gap-5 lg:grid-cols-3">{liveReviews.reviews.slice(0, 6).map((review, index) => <article key={`${review.customer_name}-${index}`} className="flex min-h-52 flex-col rounded-2xl border border-line bg-surface-muted p-5">
          <div className="flex items-center justify-between gap-3"><span className="font-black text-ink">{review.customer_name}</span><div className="flex" role="img" aria-label={`${review.rating} out of 5 stars`}>{Array.from({ length: 5 }, (_, star) => <Star key={star} size={15} className={star < review.rating ? "fill-amber-400 text-amber-400" : "text-line"}/>)}</div></div>
