...
export default function TourPageShell({ tour, locale = "en", relatedTourCandidates = tours }: { tour: Tour; locale?: Locale; relatedTourCandidates?: Tour[] }) {
  ...
  const pl = locale === "pl";
  const ui = {
    en: { experience: "experience", reviews: "customer reviews", reserve: "Book direct with local support", pickup: "See the price before you send your request. We confirm pickup details and help with changes on WhatsApp.", reviewEyebrow: "Customer reviews", reviewTitle: "Already travelled with us?", reviewText: "Share a review of your trip to help future guests choose their experience.", reviewCta: "Review this trip", before: "Before you book", faq: "Frequently asked questions", more: "More to explore", related: "Related experiences", relatedText: "Find another Red Sea excursion, diving day, desert safari, or private transfer that fits your plans.", from: "From", all: "Explore all tours" },
    ar: { experience: "تجربة", reviews: "تقييمات العملاء", reserve: "احجز مباشرة مع دعم محلي", pickup: "اطّلع على السعر قبل إرسال طلبك. نؤكد تفاصيل الاستلام ونساعدك في التغييرات عبر واتساب.", reviewEyebrow: "تقييمات العملاء", reviewTitle: "هل سافرت معنا من قبل؟", reviewText: "شارك تقييم رحلتك لمساعدة الضيوف الآخرين في اختيار تجربتهم.", reviewCta: "قيّم هذه الرحلة", before: "قبل الحجز", faq: "الأسئلة الشائعة", more: "اكتشف المزيد", related: "تجارب مشابهة", relatedText: "اختر رحلة أخرى في البحر الأحمر أو يوم غوص أو سفاري صحراوي أو انتقالًا خاصًا يناسب خطتك.", from: "ابتداءً من", all: "استكشف كل الرحلات" },
    de: { experience: "Erlebnis", reviews: "Kundenbewertungen", reserve: "Direkt buchen mit Betreuung vor Ort", pickup: "Sieh den Preis vor deiner Anfrage. Abholung und Änderungen bestätigen wir per WhatsApp.", reviewEyebrow: "Kundenbewertungen", reviewTitle: "Schon mit uns gereist?", reviewText: "Teile deine Bewertung und hilf künftigen Gästen bei der Auswahl.", reviewCta: "Ausflug bewerten", before: "Vor der Buchung", faq: "Häufig gestellte Fragen", more: "Mehr entdecken", related: "Ähnliche Erlebnisse", relatedText: "Finde einen weiteren Ausflug ans Rote Meer, einen Tauchtag, eine Wüstensafari oder einen privaten Transfer.", from: "Ab", all: "Alle Ausflüge entdecken" },
    ru: { experience: "Экскурсия", reviews: "отзывов гостей", reserve: "Бронируйте напрямую с местной поддержкой", pickup: "Цена видна до отправки заявки. Трансфер и изменения подтверждаем через WhatsApp.", reviewEyebrow: "Отзывы клиентов", reviewTitle: "Уже путешествовали с нами?", reviewText: "Оставьте отзыв о поездке и помогите будущим гостям выбрать экскурсию.", reviewCta: "Оценить поездку", before: "Перед бронированием", faq: "Частые вопросы", more: "Больше впечатлений", related: "Похожие экскурсии", relatedText: "Выберите морскую прогулку, дайвинг, сафари в пустыне или частный трансфер.", from: "От", all: "Все экскурсии" },
    pl: { experience: "Atrakcja", reviews: "opinii klientów", reserve: "Rezerwuj bezpośrednio z lokalnym wsparciem", pickup: "Cenę widzisz przed wysłaniem zgłoszenia. Odbiór i zmiany potwierdzamy przez WhatsApp.", reviewEyebrow: "Opinie klientów", reviewTitle: "Podróżowaliście już z nami?", reviewText: "Podziel się opinią z wycieczki i pomóż przyszłym gościom wybrać atrakcję.", reviewCta: "Oceń wycieczkę", before: "Przed rezerwacją", faq: "Najczęstsze pytania", more: "Odkryj więcej", related: "Podobne atrakcje", relatedText: "Znajdź inny rejs po Morzu Czerwonym, dzień nurkowania, safari lub prywatny transfer.", from: "Od", all: "Zobacz wszystkie wycieczki" },
    zh: { experience: "体验", reviews: "条客户评价", reserve: "直接预订，享受本地支持", pickup: "提交请求前即可查看价格。我们会通过 WhatsApp 确认接送并协助处理变更。", reviewEyebrow: "客户评价", reviewTitle: "已经和我们一起旅行过？", reviewText: "分享您的行程评价，帮助其他客人选择合适的体验。", reviewCta: "评价此行程", before: "预订须知", faq: "常见问题", more: "探索更多", related: "相关体验", relatedText: "寻找适合您的其他红海行程、潜水日、沙漠探险或私人接送。", from: "起价", all: "探索所有行程" },
  }[locale];
  const pausedCopy = {
  ...
  const hasCatalogReviews = Number.isFinite(reviewCount) && reviewCount > 0 && Number.isFinite(Number(tour.rating));
  const tourSchema = { "@type": "TouristTrip", "@id": `${tourUrl}#tour`, name: tour.title, description: tour.description, image: absoluteUrl(tour.image), url: tourUrl, inLanguage: locale, touristType: tour.category || `${destination?.name || "Red Sea"} excursion`, ...(hasCatalogReviews ? { aggregateRating: { "@type": "AggregateRating", ratingValue: Number(tour.rating), reviewCount, bestRating: 5, worstRating: 1 } } : {}), ...(tour.bookingMode === "inquiry" ? {} : { offers: { "@type": "Offer", price: tour.price, priceCurrency: tour.currency || "USD", availability: tour.listingStatus === "paused" ? "https://schema.org/OutOfStock" : "https://schema.org/InStock", url: tourUrl } }), provider: { "@id": `${absoluteUrl()}#organization`, "@type": "TravelAgency", name: siteName, url: absoluteUrl() } };
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div id="book" className="scroll-mt-24 lg:sticky lg:top-24 lg:self-start">
            ...
            {tour.bookingMode === "inquiry" ? <div className="rounded-3xl border border-ocean-soft bg-white p-7 shadow-sm"><p className="text-sm font-bold uppercase tracking-[0.22em] text-ocean-dark">{ui.more}</p><h2 className="mt-3 text-3xl font-black text-ink">{ui.related}</h2><p className="mt-4 leading-7 text-muted">{ui.relatedText}</p><a href={whatsappUrl(`Hello Daily Red Sea, please send me the current price and availability for ${tour.title}.`)} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex w-full justify-center rounded-2xl bg-emerald-600 px-6 py-4 font-bold text-white hover:bg-emerald-700">WhatsApp</a></div> : <><div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><p className="font-bold">{ui.reserve}</p><p className="mt-1">{ui.pickup}</p></div><Suspense fallback={<div className="min-h-[620px] rounded-3xl border bg-white shadow-sm" />}>
              {tour.listingStatus === "paused" ? <div className="min-h-[320px] rounded-3xl border border-amber-200 bg-amber-50 p-7 shadow-sm"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-800">{pausedCopy[0]}</p><h2 className="mt-3 text-3xl font-black text-ink">{pausedCopy[1]}</h2><p className="mt-4 leading-7 text-ink">{pausedCopy[2]}</p></div> : tour.slug === "hurghada-airport-transfer" ? <AirportTransferForm /> : transferService ? <TransferBookingForm initialService={transferService} /> : <BookingForm
                  tourName={tour.title}
                  tourSlug={tour.slug}
