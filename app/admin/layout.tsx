...
import { adminRoles, isAdminOwner, isAuthorizedAdmin, type AdminPermission, type AdminRole } from "@/lib/admin-auth";
...
export default async function AdminLayout({ children }: LayoutProps<"/admin">) {
  ...
  const permissionChecks = await Promise.all(allPermissions.map((permission) => hasLivePermission(supabase, user, permission)));
  const permissions = allPermissions.filter((_, index) => permissionChecks[index]);
