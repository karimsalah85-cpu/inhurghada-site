...
const integrations = [
  ...
  { name: "Email", keys: ["GMAIL_SMTP_APP_PASSWORD", "RESEND_API_KEY"], anyKey: true },
  { name: "WhatsApp", keys: ["TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN", "TWILIO_WHATSAPP_FROM"] },
] as const;

export default async function IntegrationsPage() {
  await requireAdminPage("settings");
  return <AdminPageFrame eyebrow="System & access" title="Integrations" description="Connection health for this environment. Credentials remain in protected deployment variables and are never displayed here.">
    <div className="grid gap-4 md:grid-cols-2">{integrations.map((integration) => {
      const connected = "anyKey" in integration && integration.anyKey
        ? integration.keys.some((key) => Boolean(process.env[key]))
