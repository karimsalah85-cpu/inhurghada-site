...
export default async function LocalizedPage({ params }: LocalizedPageProps) {
  ...
  if (kind === "category") {
    ...
    const relatedDivingCopy = {
      en: { title: "Discover diving and snorkeling in Hurghada", text: "Compare guided experiences, from a first dive to full-day reef trips.", cta: "Explore diving and snorkeling" },
      ar: { title: "اكتشف الغوص والسنوركلينج في الغردقة", text: "قارن بين التجارب المصحوبة بمرشد، من الغوص لأول مرة إلى رحلات الشعاب المرجانية ليوم كامل.", cta: "استكشف الغوص والسنوركلينج" },
      de: { title: "Tauchen und Schnorcheln in Hurghada entdecken", text: "Vergleiche geführte Erlebnisse vom ersten Tauchgang bis zum ganztägigen Riffausflug.", cta: "Tauchen und Schnorcheln entdecken" },
      ru: { title: "Дайвинг и снорклинг в Хургаде", text: "Сравните экскурсии с инструктором: от первого погружения до поездок к рифам на целый день.", cta: "Выбрать дайвинг или снорклинг" },
      ...
      zh: { title: "探索赫尔格达潜水与浮潜", text: "比较有向导的体验，从首次潜水到全天珊瑚礁之旅。", cta: "探索潜水与浮潜" },
    }[locale];
    return <Shell locale={locale}><main dir={direction} className="bg-slate-50 pb-20"><section className="bg-slate-950 px-6 py-20 text-white"><div className="mx-auto max-w-5xl"><p className="font-bold text-cyan-300">Daily Red Sea · Hurghada</p><h1 className="mt-4 text-4xl font-black sm:text-6xl">{categoryLabels[locale][category.slug]}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">{dictionary.siteDescription}</p></div></section><section className="mx-auto max-w-7xl px-6 py-16"><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{categoryTours.map((tour) => <Link key={tour.slug} href={localePath(locale, `/tours/${tour.slug}`)} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="relative h-52"><Image src={tour.image} alt={localizedTourTitle(locale, tour.slug, tour.title)} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" /></div><div className="p-6"><p className="text-sm text-slate-500">{tour.duration}</p><h2 className="mt-2 text-xl font-black">{localizedTourTitle(locale, tour.slug, tour.title)}</h2><p className="mt-4 font-black text-blue-700">{dictionary.from} {tour.originalPrice && Number(tour.originalPrice) > Number(tour.price) ? <span className="mr-2 text-slate-400 line-through">${tour.originalPrice}</span> : null}${tour.price} · {dictionary.perPerson}</p><span className="mt-5 inline-block font-bold text-cyan-700">{dictionary.bookNow} →</span></div></Link>)}</div></section>{category.slug !== "diving-snorkeling" ? <section className="border-t border-slate-200 bg-cyan-50 px-6 py-16"><div className="mx-auto max-w-4xl rounded-3xl border border-cyan-200 bg-white p-8"><h2 className="text-3xl font-black text-slate-950">{relatedDivingCopy.title}</h2><p className="mt-4 max-w-2xl leading-8 text-slate-600">{relatedDivingCopy.text}</p><Link href={localePath(locale, "/hurghada/diving-snorkeling")} className="mt-6 inline-block rounded-full bg-cyan-700 px-6 py-3 font-bold text-white">{relatedDivingCopy.cta} →</Link></div></section> : null}</main></Shell>;
  }
