  ...
  return (
    ...
    <>
      ...
      <section className="bg-slate-50 px-6 py-20 sm:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="font-semibold uppercase tracking-[0.24em] text-blue-600">{de ? "Die besten Aktivitäten am Roten Meer" : ru ? "Лучшие развлечения на Красном море" : ar ? "أفضل الأنشطة في البحر الأحمر" : pl ? "Najlepsze atrakcje nad Morzem Czerwonym" : zh ? "红海热门体验" : "Best things to do on the Red Sea"}</p>
            <h2 className="mt-3 text-4xl font-black text-slate-900">{de ? "Erlebnisse für jeden Reiseplan" : ru ? "Впечатления для любого плана" : ar ? "تجارب تناسب كل خطة" : pl ? "Atrakcje na każdy plan podróży" : zh ? "适合每种行程的红海体验" : "Red Sea experiences for every plan"}</h2>
            <p className="mt-4 text-lg leading-8 text-slate-600">{de ? "Türkisblaues Wasser, goldene Wüstendünen oder eine bequeme Fahrt von Tür zu Tür – dein perfekter Tag in Hurghada oder Marsa Alam beginnt hier." : ru ? "Бирюзовое море, золотые дюны или удобный трансфер от двери до двери — ваш идеальный день в Хургаде или Марса-Аламе начинается здесь." : ar ? "مياه فيروزية وكثبان ذهبية وتنقل مريح من الباب إلى الباب — يومك المثالي في الغردقة أو مرسى علم يبدأ هنا." : pl ? "Turkusowa woda, złote wydmy albo wygodny przejazd od drzwi do drzwi — Twój idealny dzień w Hurghadzie lub Marsa Alam zaczyna się tutaj." : zh ? "碧绿海水、金色沙丘或舒适的门到门接送，您的赫尔格达或马萨阿拉姆之旅从这里开始。" : "Chase turquoise water, golden-hour dunes, or a smooth door-to-door ride—your best day in Hurghada or Marsa Alam starts here."}</p>
          </div>
