  ...
  const description = normalizeMetaDescription(
    destination.slug === "hurghada"
      ...
      : destination.comingSoon
        ...
        : "Book Marsa Alam snorkeling trips to Dolphin House, Marsa Mubarak and Abu Dabbab with clear EUR prices, included hotel pickup and local support.",
  );
  return {
    title,
    description,
...
export default async function DestinationPage({ params }: Props) {
  ...
  const destinationTours = tours.filter((tour) => (tour.destinationSlug || "hurghada") === destination.slug && tour.listingStatus !== "paused" && tour.listingStatus !== "unlisted");
  return <main className="min-h-screen bg-slate-50 px-6 pb-20 pt-32 sm:px-8"><div className="mx-auto max-w-7xl"><p className="font-semibold uppercase tracking-[0.24em] text-cyan-700">{destination.country}</p><h1 className="mt-4 text-5xl font-black text-slate-950">Things to do in {destination.name}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">{destination.tagline}. Explore {destinationTours.length} bookable experiences with clear pricing and local support.</p><div className="mt-12"><CategoryTourExplorer tours={destinationTours} /></div></div></main>;
}
