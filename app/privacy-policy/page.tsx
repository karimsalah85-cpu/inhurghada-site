...
export default function PrivacyPolicyPage({ locale = "en" }: { locale?: "en" | "de" | "ru" | "ar" | "pl" | "zh" }) {
  const de = locale === "de";
  const ru = locale === "ru";
  const ar = locale === "ar";
  const pl = locale === "pl";
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-24 lg:px-8">
      <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm sm:p-12">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-700">{de ? "Rechtliches" : ru ? "Правовая информация" : ar ? "معلومات قانونية" : pl ? "Informacje prawne" : zh ? "法律信息" : "Legal"}</p>
        <h1 className="mt-4 text-4xl font-black text-slate-900">{de ? "Datenschutzerklärung" : ru ? "Политика конфиденциальности" : ar ? "سياسة الخصوصية" : pl ? "Polityka prywatności" : zh ? "隐私政策" : "Privacy Policy"}</h1>
        ...
        <div className="mt-10 space-y-6 text-slate-700">
          <section>
            <h2 className="text-xl font-bold text-slate-900">{de ? "Erhobene Informationen" : ru ? "Какие данные мы собираем" : ar ? "المعلومات التي نجمعها" : pl ? "Gromadzone informacje" : zh ? "我们收集的信息" : "Information we collect"}</h2>
          ...
          <section>
            <h2 className="text-xl font-bold text-slate-900">{de ? "Verwendung der Daten" : ru ? "Как мы используем данные" : ar ? "كيف نستخدم البيانات" : pl ? "Jak wykorzystujemy dane" : zh ? "信息用途" : "How we use it"}</h2>
          ...
          <section>
            <h2 className="text-xl font-bold text-slate-900">{de ? "Kontakt" : ru ? "Контакты" : ar ? "تواصل معنا" : pl ? "Kontakt" : zh ? "联系我们" : "Contact"}</h2>
        ...
        <Link href={localePath(locale)} className="mt-10 inline-flex text-sm font-semibold text-cyan-700">← {de ? "Zur Startseite" : ru ? "На главную" : ar ? "العودة إلى الرئيسية" : pl ? "Powrót na stronę główną" : zh ? "返回首页" : "Back to home"}</Link>
      </div>
    </main>
  );
}
