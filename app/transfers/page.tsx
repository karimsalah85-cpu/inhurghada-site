    ...
    <main className="bg-white">
      ...
      <section id="book-transfer" className="bg-surface-muted px-6 py-20">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
          <div>
            <p className="font-semibold uppercase tracking-[0.24em] text-ocean">{de ? "Private Transfers" : ru ? "Частные трансферы" : ar ? "تنقلات خاصة" : pl ? "Prywatne transfery" : zh ? "私人接送" : "Private transfers"}</p>
      ...
      {locale === "en" ? <section className="border-t border-line bg-white px-6 py-20"><div className="mx-auto max-w-4xl"><p className="font-semibold uppercase tracking-[0.24em] text-ocean">Plan your pickup</p><h2 className="mt-3 text-4xl font-bold text-ink">Hurghada transfer prices and questions</h2><p className="mt-5 text-lg leading-8 text-muted">One-way private airport transfers start at $20 within Hurghada. Makadi Bay, Soma Bay, El Gouna and Sahl Hasheesh add $7. Your final vehicle, pickup point and time are confirmed before travel.</p><div className="mt-8 divide-y divide-line rounded-3xl border border-line px-6">{transferFaqs.map((item) => <details key={item.question} className="py-5"><summary className="cursor-pointer font-bold text-ink">{item.question}</summary><p className="mt-3 leading-7 text-muted">{item.answer}</p></details>)}</div></div></section> : null}
