...
export default async function TourCategoryPage({ params, locale = "en" }: PageProps & { locale?: "en" | "de" | "ru" | "ar" | "pl" | "zh" }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      {category.slug === "diving-snorkeling" && locale === "en" ? <section className="border-t border-slate-200 bg-cyan-50 px-6 py-16 sm:px-8"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-black text-slate-950">Hurghada diving and snorkeling guides</h2><p className="mt-4 max-w-3xl leading-8 text-slate-600">Use these local planning guides to choose the right Red Sea day before you book.</p><div className="mt-7 grid gap-4 md:grid-cols-2"><Link href="/blog/best-snorkeling-tours-in-hurghada-for-beginners" className="rounded-3xl border border-cyan-200 bg-white p-6 transition hover:border-cyan-500"><h3 className="text-xl font-black text-slate-950">Best snorkeling tours in Hurghada for beginners</h3><p className="mt-3 leading-7 text-slate-600">Compare reef-focused, island and wildlife-led days by confidence level and time in the water.</p><span className="mt-4 inline-block font-bold text-cyan-800">Read the guide →</span></Link><Link href="/tours/full-day-diving" className="rounded-3xl border border-cyan-200 bg-white p-6 transition hover:border-cyan-500"><h3 className="text-xl font-black text-slate-950">Full-day scuba diving trip</h3><p className="mt-3 leading-7 text-slate-600">See two guided dives, lunch, pickup and the experience requirements before booking.</p><span className="mt-4 inline-block font-bold text-cyan-800">View full-day diving →</span></Link></div></div></section> : null}
