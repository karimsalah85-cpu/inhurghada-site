    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="bg-slate-950 px-6 pb-20 pt-32 text-white sm:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-sm font-bold"><span className="rounded-full bg-white/10 px-4 py-2">{de ? "Klare lokale Preise" : ru ? "Понятные местные цены" : "Clear local prices"}</span><span className="rounded-full bg-white/10 px-4 py-2">{de ? "Bestätigung per WhatsApp" : ru ? "Подтверждение в WhatsApp" : "WhatsApp confirmation"}</span><span className="rounded-full bg-white/10 px-4 py-2">{de ? "Barzahlung bei Ankunft" : ru ? "Оплата наличными по прибытии" : "Cash on arrival"}</span></div>
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-6 py-16 sm:px-8">
        <CategoryTourExplorer tours={categoryTours} locale={locale} />
      </section>
      <section className="border-t border-slate-200 bg-white px-6 py-16 sm:px-8"><div className="mx-auto max-w-4xl"><h2 className="text-3xl font-black text-slate-950">{de ? "Mit klaren Informationen buchen" : ru ? "Бронируйте с полной информацией" : "Book with clear information"}</h2><p className="mt-4 leading-8 text-slate-600">{de ? "Jeder Ausflug zeigt Startpreis, Dauer, Abholung, enthaltene Leistungen und wichtige Hinweise vor der Buchung. Unser Team bestätigt die endgültigen Details direkt per WhatsApp." : ru ? "Перед бронированием вы увидите начальную цену, продолжительность, информацию о трансфере, включённые услуги и важные примечания. Наша местная команда подтвердит окончательные детали в WhatsApp." : "Every Daily Red Sea experience shows its starting price, duration, pickup information, inclusions, and practical notes before you submit a booking. Our local team confirms final pickup details directly on WhatsApp."}</p></div></section>
    </main>
  );
}
