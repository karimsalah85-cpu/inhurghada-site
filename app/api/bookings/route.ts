...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: reservation, error: bookingError } = await supabase.rpc("reserve_booking_idempotent", {
      ...
      p_tour_slug: body.tourSlug || (bookingType === "transfer" ? (body.transfer ? "hurghada-airport-transfer" : body.service) : ""),
      p_date: body.date,
      p_start_time: body.time || null,
      p_guests: guestCount,
      p_adults: bookingType === "tour" ? body.adults : body.transfer ? body.transfer.adults : 0,
      p_youth: bookingType === "tour" ? body.youth : body.transfer ? body.transfer.children : 0,
      p_infants: bookingType === "tour" ? body.infants : body.transfer ? body.transfer.infants : 0,
      p_hotel: hotel,
