...
export async function GET() {
  ...
  const results = await Promise.all([
    ...
    supabase.from("bookings").select("id,reference,customer_name,customer_email,phone,tour_name,tour_slug,date,guests,amount,currency,status,payment_status,created_at").order("date").limit(1000),
