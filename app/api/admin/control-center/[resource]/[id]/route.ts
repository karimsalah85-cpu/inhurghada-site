...
export async function PATCH(request: NextRequest, context: { params: Promise<{ resource: string; id: string }> }) {
  ...
  const allowed: Record<string, string[]> = {
    content: ["trip_id", "content_type", "slug", "locale", "status", "listing_status", "title", "excerpt", "body", "seo_title", "seo_description", "canonical_path", "featured_image", "publish_at"],
  ...
  if (resource === "content" && Object.hasOwn(update, "trip_id")) {
    try { update.trip_id = normalizeTripId(update.trip_id); }
    catch (error) { return NextResponse.json({ error: (error as Error).message }, { status: 400 }); }
    if (update.trip_id && ((update.content_type ?? before.content_type) !== "tour" || (update.locale ?? before.locale) !== "en")) return NextResponse.json({ error: "Set the Trip ID on the English trip; it is shared across languages." }, { status: 400 });
  }
