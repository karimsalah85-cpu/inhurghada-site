...
export async function getLiveTours(locale = "en"): Promise<Tour[]> {
  const rows = await contentRows("tour");
  if (!rows.length) return applyTourMedia(tours, locale);
  ...
  return applyTourMedia([...tours.filter((tour) => !managedSlugs.has(tour.slug)), ...overrides.values()], locale);
}
