...
type ContentRow = { slug: string; title: string; excerpt: string | null; body: unknown; seo_title: string | null; seo_description: string | null; featured_image: string | null; published_at: string | null; publish_at: string | null };

async function publishedRows(contentType: "tour" | "blog") {
  const client = createAdminClient();
  if (!client) return [];
  const { data, error } = await client.from("content_items").select("slug,title,excerpt,body,seo_title,seo_description,featured_image,published_at,publish_at").eq("content_type", contentType).eq("locale", "en").eq("status", "published");
  if (error) {
...
export async function getLiveTours(): Promise<Tour[]> {
  const rows = await publishedRows("tour");
...
export async function getLiveBlogPosts(): Promise<BlogPost[]> {
  ...
  return [...blogPosts.filter((post) => !overrides.has(post.slug)), ...overrides.values()];
}
